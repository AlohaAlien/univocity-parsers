 ...
 * {@link CommonWriterSettings} and currently does not require any additional
 * setting.
 *
 * <p>
 * The FixedWidthParserSettings requires a definition of the field lengths of
 * each record in the input. This must provided using an instance of
...
public class FixedWidthWriterSettings extends CommonWriterSettings<FixedWidthFormat> {
	...
	/**
	 * Creates a basic configuration object for the Fixed-Width writer with no field
	...
	/**
	 * Returns the sequence of field alignments to apply to each field in the
	 * record.
	 *
	 * @return the sequence of field alignments to apply to each field in the
	 *         record.
	 */
	 ...
	 *                  will notify the writer to switch to a new record format,
	 ...
	 *
	 * @param lookbehind the lookbehind value that when present in the previous
	 *                   output row,
	 *                   will notify the writer to switch to a new record format,
	 *                   with different field lengths
	 * @param lengths    the field lengths of the record format identified by the
	...
	/**
	 * Indicates whether headers should be written using the default padding
	 ...
	 *
	 * @return {@code true} if the default padding is to be used when writing
	...
	/**
	 * Defines whether headers should be written using the default padding specified
	 * in {@link FixedWidthFormat#getPadding()}
	 * instead of any custom padding associated with a given field (in
	 * {@link FixedWidthFields#setPadding(char, int...)})
	 *
	 * @param useDefaultPaddingForHeaders flag indicating whether the default
	 *                                    padding is to be used when writing headers
	 */
	...
	/**
	 * Returns the default alignment to use when writing headers. If none is
	 * specified (i.e. {@code null}), the headers will be aligned
	 * according to the corresponding field alignment in
	 * {@link FixedWidthFields#getAlignment(String)}.
	 *
	 * Defaults to {@code null}.
	 *
	 * @return the default alignment for headers, or {@code null} if the headers
	...
	/**
	 * Defines the default alignment to use when writing headers. If none is
	 * specified (i.e. {@code null}), the headers will be aligned
	 * according to the corresponding field alignment in
	 * {@link FixedWidthFields#getAlignment(String)}.
	 *
	 * Defaults to {@code null}.
	 *
	 * @param defaultAlignmentForHeaders the alignment to use when writing headers.
	 *                                   {@code null} indicates that headers
	 *                                   should be aligned according to the column
	 *                                   alignment.
	 */
	...
	/**
	 * Returns a flag indicating whether each record, when written, should be
	 * followed by a line separator (as specified in
	 * {@link Format#getLineSeparator()}.
	 *
	 * Consider the records {@code [a,b]} and {@code [c,d]}, with field lengths
	 * {@code [2, 2]}, and line separator = {@code \n}:
	 * <ul>
	 * <li>When {@link #getWriteLineSeparatorAfterRecord()} is enabled, the output
	 * will be written as: {@code a b \nc d \n}</li>
	 * <li>When {@link #getWriteLineSeparatorAfterRecord()} is disabled, the output
	 * will be written as: {@code a b c d }</li>
	 * </ul>
	 *
	 * Defaults to {@code true}.
	 *
	 * @return whether the writer should add a line separator after a record is
	...
	/**
	 * Defines whether each record, when written, should be followed by a line
	 * separator (as specified in {@link Format#getLineSeparator()}.
	 *
	 * Consider the records {@code [a,b]} and {@code [c,d]}, with field lengths
	 * {@code [2, 2]}, and line separator = {@code \n}:
	 * <ul>
	 ...
	 * Defaults to {@code true}.
	 *
	 * @param writeLineSeparatorAfterRecord flag indicating whether the writer
	 *                                      should add a line separator after a
	 *                                      record is written to the output.
	 */
	 ...
	 * @return a copy of all configurations applied to the current instance.
	 *
	 * @deprecated doesn't really make sense for fixed-width. . Use alternative
	 *             method {@link #clone(FixedWidthFields)}.
	 */
	@Deprecated

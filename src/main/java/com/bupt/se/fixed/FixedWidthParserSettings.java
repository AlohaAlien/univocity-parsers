 ...
 * ({@link FixedWidthParser})
 *
 * <p>
 * In addition to the configuration options provided by
 * {@link CommonParserSettings}, the FixedWidthParserSettings include:
 *
 * <ul>
 * <li><b>skipTrailingCharsUntilNewline <i>(defaults to {@code false})</i>:</b>
 * Indicates whether or not any trailing characters beyond the record's length
 * should be skipped until the newline is reached
 * <p>
 * For example, if the record length is 5, but the row contains "12345678\n",
 * then portion containing "678" will be discarded and not considered part of
 * the next record</li>
 * <li><b>recordEndsOnNewline <i>(defaults to {@code false})</i>:</b> Indicates
 * whether or not a record is considered parsed when a newline is reached.
 * <p>
 * For example, if recordEndsOnNewline is set to true, then given a record of
 * length 4, and the input "12\n3456", the parser will identify [12] and [3456]
 * <p>
 * If recordEndsOnNewline is set to false, then given a record of length 4, and
 * the input "12\n3456", the parser will identify a multi-line record [12\n3]
 * and [456 ]</li>
 * </ul>
 *
 * <p>
 * The FixedWidthParserSettings need a definition of the field lengths of each
 * record in the input. This must provided using an instance of
 * {@link FixedWidthFields}.
 *
 * @author Univocity Software Pty Ltd -
 *         <a href="mailto:parsers@univocity.com">parsers@univocity.com</a>
 * @see com.bupt.se.fixed.FixedWidthParser
 * @see com.bupt.se.fixed.FixedWidthFormat
...
public class FixedWidthParserSettings extends CommonParserSettings<FixedWidthFormat> {
	...
	/**
	 * You can only create an instance of this class by providing a definition of
	 * the field lengths of each record in the input.
	 * <p>
	 * This must provided using an instance of {@link FixedWidthFields}.
	 *
	 * @param fieldLengths the instance of {@link FixedWidthFields} which provides
	 *                     the lengths of each field in the fixed-width records to
	 *                     be parsed
	 *
	 * @see FixedWidthFields
	 */
	...
	/**
	 * Creates a basic configuration object for the Fixed-Width parser with no field
	 * length configuration.
	 * This constructor is intended to be used when the record length varies
	 * depending of the input row.
	 * Refer to {@link #addFormatForLookahead(String, FixedWidthFields)},
	 * {@link #addFormatForLookbehind(String, FixedWidthFields)}
	 */
	...
	/**
	 * Indicates whether or not any trailing characters beyond the record's length
	 * should be skipped until the newline is reached (defaults to {@code false})
	 * <p>
	 * For example, if the record length is 5, but the row contains "12345678\n",
	 * then the portion containing "678\n" will be discarded and not considered part
	 * of the next record
	 *
	 * @return returns true if any trailing characters beyond the record's length
	 *         should be skipped until the newline is reached, false otherwise
	 */
	...
	/**
	 * Defines whether or not any trailing characters beyond the record's length
	 * should be skipped until the newline is reached (defaults to {@code false})
	 * <p>
	 * For example, if the record length is 5, but the row contains "12345678\n",
	 * then the portion containing "678\n" will be discarded and not considered part
	 * of the next record
	 *
	 * @param skipTrailingCharsUntilNewline a flag indicating if any trailing
	 *                                      characters beyond the record's length
	 *                                      should be skipped until the newline is
	 *                                      reached
	 */
	...
	/**
	 * Indicates whether or not a record is considered parsed when a newline is
	 * reached. Examples:
	 * <ul>
	 * <li>Consider two records of length <b>4</b>, and the input
	 * <b>12\n3456</b></li>
	 * <li>When {@link FixedWidthParserSettings#recordEndsOnNewline} is set to true:
	 * the first value will be read as <b>12</b> and the second <b>3456</b></li>
	 * <li>When {@link FixedWidthParserSettings#recordEndsOnNewline} is set to
	 * false: the first value will be read as <b>12\n3</b> and the second
	 * <b>456</b></li>
	 * </ul>
	 * <p>
	 * <i>defaults to {@code false}</i>
	 *
	 * @return true if a record should be considered parsed when a newline is
	...
	/**
	 * Defines whether or not a record is considered parsed when a newline is
	 * reached. Examples:
	 * <ul>
	 * <li>Consider two records of length <b>4</b>, and the input
	 * <b>12\n3456</b></li>
	 ...
	 * </ul>
	 *
	 * @param recordEndsOnNewline a flag indicating whether or not a record is
	 *                            considered parsed when a newline is reached
	 */
	 ...
	 *
	 * @return and instance of FixedWidthFormat configured to handle Fixed-Width
	 *         inputs
	 */
	@Override
	 ...
	 * characters per column and, default value used to represent a null value (when
	 * the String parsed from the input is empty), and the padding character to
	 * handle unwritten positions
	 *
	 * <p>
	 * This overrides the parent implementation to create a CharAppender capable of
	 * handling padding characters that represent unwritten positions.
	 *
	 * @return an instance of CharAppender with the configured limit of maximum
	 *         characters per column and, default value used to represent a null
	 *         value (when the String parsed from the input is empty), and the
	 *         padding character to handle unwritten positions
	 */
	@Override
	 ...
	 * written/read. Used to avoid OutOfMemoryErrors (defaults to a minimum of 4096
	 * characters).
	 *
	 * <p>
	 * This overrides the parent implementation and calculates the absolute minimum
	 * number of characters required to store the values of a record
	 * <p>
	 * If the sum of all field lengths is greater than the configured maximum number
	 * of characters per column, the calculated amount will be returned.
	 *
	...
	/**
	 * Returns the hard limit of how many columns a record can have (defaults to a
	 * maximum of 512).
	 ...
	 * <p>
	 * This overrides the parent implementation and calculates the absolute minimum
	 * number of columns required to store the values of a record
	 * <p>
	 * If the sum of all fields is greater than the configured maximum number
	 * columns, the calculated amount will be returned.
	 *
	 ...
	 *                  will notify the parser to switch to a new record format,
	 *                  with different field lengths
	 * @param lengths   the field lengths of the record format identified by the
	 *                  given lookahead symbol.
	 */
	 ...
	 *
	 * @param lookbehind the lookbehind value that when found in the previous input
	 *                   row,
	 *                   will notify the parser to switch to a new record format,
	 *                   with different field lengths
	 * @param lengths    the field lengths of the record format identified by the
	 *                   given lookbehind symbol.
	 */
	...
	/**
	 * Indicates whether headers should be parsed using the default padding
	 * specified in {@link FixedWidthFormat#getPadding()}
	 * instead of any custom padding associated with a given field (in
	 * {@link FixedWidthFields#setPadding(char, int...)})
	 * Defaults to {@code true}
	 *
	 * @return {@code true} if the default padding is to be used when reading
	 *         headers, otherwise {@code false}
	 */
	...
	/**
	 * Defines whether headers should be parsed using the default padding specified
	 * in {@link FixedWidthFormat#getPadding()}
	 * instead of any custom padding associated with a given field (in
	 * {@link FixedWidthFields#setPadding(char, int...)})
	 *
	 * @param useDefaultPaddingForHeaders flag indicating whether the default
	 *                                    padding is to be used when parsing headers
	 */
	 ...
	 * including the fixed-width field configuration.
	 *
	...
	/**
	 * Clones this configuration object to reuse most user-provided settings. This
	 * includes the fixed-width field configuration,
	 * but doesn't include other input-specific settings. This method is meant to be
	 * used internally only.
	 *
	 * @return a copy of all configurations applied to the current instance.
	 *
	 * @deprecated doesn't really make sense for fixed-width. Use alternative method
	 *             {@link #clone(FixedWidthFields)}.
	 */
	@Deprecated
	 ...
	 * input (such as header names and selection of fields) will be reset to their
	 * defaults.
	 *
	 * To obtain a full copy, use {@link #clone()}.
	 *
	 * @param fields the fixed-width field configuration to be used by the cloned
	 *               settings object.
	 *
	 * @return a copy of the <em>general</em> configurations applied to the current
	 *         instance.
	 */
	 ...
	 *
	 * @return flag indicating the padding character should be kept in the parsed
	 *         value
	 */
	...
	/**
	 * Configures the fixed-width parser to retain the padding character in any
	 * parsed values
	 *
	 * <i>(defaults to {@code false})</i>
	 *
	 ...
	 * {@link FixedWidthFields#keepPaddingOn(String, String...)}
	 *
	 * @param keepPadding flag indicating the padding character should be kept in
	 *                    the parsed value
	 */
	...
	/**
	 * Returns the sequence of fields whose padding character must/must not be
	 * retained in the parsed value
	 * 

...
public class FixedWidthWriter extends AbstractWriter<FixedWidthWriterSettings> {
	 ...
	 * {@link FixedWidthWriterSettings}, and requires this configuration to be
	 ...
	 * properly initialized.
	 *
	 * @param writer   the output resource that will receive fixed-width records
	 ...
	 * properly initialized.
	 *
	 * @param file     the output file that will receive fixed-width records
	 *                 produced by this class.
	 * @param settings the fixed-width writer configuration
	 */
	 ...
	 * properly initialized.
	 *
	 * @param output   the output stream that will be written with the fixed-width
	 *                 records produced by this class.
	 * @param settings the fixed-width writer configuration
	 */
				...
				if (lookbehindFormat == null) {
					if (rootLengths == null) {
						throw new TextWritingException(
								"Cannot write with the given configuration. No default field lengths defined and no lookahead/lookbehind value match '"
										+ new String(lookupChars) + '\'',
								getRecordCount(), row);
					}
					fieldLengths = rootLengths;
					fieldAlignments = rootAlignments;
		...
		if (allowTrim && this.ignoreTrailing) {
			...
			while (i < element.length() && length > 0) {
				...
				if (length == -1 && appender.whitespaceCount() > 0) {
					// if we got here then the value to write got truncated exactly after one or
					// more whitespaces.
					// In this case, if the whitespaces are not at the end of the truncated value
					// they will be put back to the output.
					for (int j = i; j < element.length(); j++) {
						if (element.charAt(j) > ' ') {
							// resets the whitespace count so the original whitespaces are printed to the
							// output.

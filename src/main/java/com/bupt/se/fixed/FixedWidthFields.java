...
/**
 * This class provides the name, length, alignment and padding of each field in
...
public class FixedWidthFields implements Cloneable {
	...
	/**
	 * Defines a sequence of field names used to refer to columns in the
	 * input/output text of an entity, along with their lengths.
	 * The field names defined will be used as headers, having the same effect of a
	 * call to {@link FixedWidthParserSettings#setHeaders(String...)}.
	 *
	 * @param fields a {@link LinkedHashMap} containing the sequence of fields to be
	 *               associated with each column in the input/output, with their
	 *               respective length.
	 */
	 ...
	 * call to {@link FixedWidthParserSettings#setHeaders(String...)}.
	 *
	 * @param headers the sequence of fields to be associated with each column in
	 *                the input/output
	 * @param lengths the sequence of lengths to be associated with each given
	 *                header. The size of this array must match the number of given
	 *                headers.
	 */
	public FixedWidthFields(String[] headers, int[] lengths) {
		...
		if (headers.length != lengths.length) {
			throw new IllegalArgumentException("Sequence of headers and their respective lengths must match. Got "
					+ headers.length + " headers but " + lengths.length + " lengths");
		}
	...
	/**
	 * Creates a new instance initialized with the lengths of all fields in a
	 * fixed-width record.
	 *
	 * @param fieldLengths The number lengths of all fields in a fixed-width record.
	...
	/**
	 * Creates a new instance initialized from {@link FixedWidth} annotations in the
	 * fields and methods of a given class. Note that
	 * all fields should additionally have the {@link Parsed} annotation to
	 * configure header names and/or their positions.
	 *
	 * @param beanClass the class whose {@link FixedWidth} annotations will be
	 *                  processed to configure this field list.
	 *
	 * @deprecated use {@link #forParsing(Class)} and {@link #forWriting(Class)} to
	 *             initialize the fields from the given
	 *             class and filter out getters and setters that target the same
	 *             field. If the given class has any annotated methods
	 ...
	 *                  processed to configure this field list.
	 *
	 * @return a new {@link FixedWidthFields} instance built with the
	 *         {@link FixedWidth} annotations found in the given class' attributes
	 *         and methods (excluding getters)
	 */
	...
	/**
	 * Creates a new instance initialized from {@link FixedWidth} annotations in the
	 * fields of a given class. Note that
	 * all fields should additionally have the {@link Parsed} annotation to
	 * configure header names and/or their positions.
	 *
	 * @param beanClass    the class whose {@link FixedWidth} annotations will be
	 *                     processed to configure this field list.
	 * @param methodFilter filter to apply over annotated methods when the
	 *                     fixed-width writer is reading data from beans (to write
	 *                     values to an output)
	...
	private FixedWidthFields(Class beanClass, MethodFilter methodFilter) {
			...
			if (length != -1) {
				if (from != -1 || to != -1) {
					throw new IllegalArgumentException(
							"Can't initialize fixed-width field from " + field.describe() + ". " +
									"Can't have field length (" + length + ") defined along with position from (" + from
									+ ") and to (" + to + ")");
		...
		if (fieldNamesWithoutConfig.size() > 0) {
			throw new IllegalArgumentException(
					"Can't derive fixed-width fields from class '" + beanClass.getName() + "'. " +
							"The following fields don't have a @FixedWidth annotation: " + fieldNamesWithoutConfig);
		}
	}

	/**
	 * Adds the range of the next field in a fixed-width record. The given range
	 * cannot overlap with previously defined fields.
	...
	public FixedWidthFields addField(String name, int startPosition, int endPosition, FieldAlignment alignment,
		...
		if (startPosition < totalLength) {
			throw new IllegalArgumentException(
					"Start position '" + startPosition + "' overlaps with one or more fields");
	...
	/**
	 * Adds the length of the next field in a fixed-width record. This method can be
	 * chained like this: addField(5).addField(6)...
	 *
	...
	/**
	 * Adds the length of the next field in a fixed-width record. This method can be
	 * chained like this: addField("field_1", 5).addField("field_2", 6)...
	 *
	 * @param name   the name of the next field. It is not validated.
	 * @param length the length of the next field. It must be greater than 0.
	 *
	...
	private void validateLength(String name, int length) {
		if (length < 1) {
			if (name == null) {
				throw new IllegalArgumentException(
						"Invalid field length: " + length + " for field at index " + fieldLengths.size());
			} else {
	 ...
	 *
	 * @return the name of each field in a fixed-width record, or null if no name
	 *         has been defined.
	 */
	...
	/**
	 * Returns the index of a field name. An {@code IllegalArgumentException} will
	 * be thrown if no names have been defined.
	 *
	...
	/**
	 * Returns the alignment of a given field. An {@code IllegalArgumentException}
	 * will be thrown if no names have been defined.
	 *
	...
	public FieldAlignment getAlignment(String fieldName) {
		...
		if (index == -1) {
			throw new IllegalArgumentException(
					"Field '" + fieldName + "' does not exist. Available field names are: " + this.fieldNames);
		}
		return getAlignment(index);
	}

	/**
	 * Returns a copy of the sequence of alignment settings to apply over each field
	 * in the fixed-width record.
	 *
	 * @return the sequence of alignment settings to apply over each field in the
	 *         fixed-width record.
	 */
	...
	/**
	 * Returns a copy of the sequence of padding characters to apply over each field
	 * in the fixed-width record.
	 *
	 * The null character ({@code '\0'}) is used to inform no padding has been
	 * explicitly set for a field, and that the
	 * default padding character defined in {@link FixedWidthFormat#getPadding()}
	 * should be used.
	 *
	 * @return the sequence of padding characters to apply over each field in the
	 *         fixed-width record.
	 */
	 ...
	 * @param positions the positions of the fields that should use the given
	 *                  padding character
	 */
	 ...
	 *
	 * @param padding the padding to apply
	 * @param names   the names of the fields that should use the given padding
	 *                character
	 */
	...
	/**
	 * Configures a given list of fields to retain the padding character in any
	 * parsed values,
	 * overriding the any default set for the whole input in
	 * {@link FixedWidthParserSettings#getKeepPadding()}
	 *
	 * @param position  the positions of the fields that should keep the padding
	 *                  character
	 ...
	 * {@link FixedWidthParserSettings#getKeepPadding()}
	 *
	 * @param name  the name of the first field that should keep the padding
	 *              character
	...
	/**
	 * Configures a given list of fields to remove the padding character in any
	 * parsed values,
	 * overriding the any default set for the whole input in
	 * {@link FixedWidthParserSettings#getKeepPadding()}
	 *
	 ...
	 * {@link FixedWidthParserSettings#getKeepPadding()}
	 *
	 * @param name  the name of the first field that have any padding characters
	 *              removed
	 * @param names the names of the fields that should have any padding characters
	 *              removed
	 */

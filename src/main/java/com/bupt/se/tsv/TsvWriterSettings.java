 ...
 *
 * <p>
 * It does not offer additional configuration options on top of the ones
 * provided by the {@link CommonWriterSettings}
 * </p>
 *
 * @see com.bupt.se.tsv.TsvWriter
 * @see com.bupt.se.tsv.TsvFormat
...
public class TsvWriterSettings extends CommonWriterSettings<TsvFormat> {
	...
	/**
	 * Identifies whether values containing line endings should have the line
	 * separator written using
	 * the escape character (defined by {@link TsvFormat#getEscapeChar()} followed
	 * by the actual line separator character
	 * instead of other characters such as the standard letters 'n' and 'r'
	 *
	 * When line joining is disabled (the default), the {@link TsvWriter} will
	 * convert new line characters into
	 * sequences containing the escape character (typically '\') followed by
	 * characters 'n' or 'r'.
	 * No matter how many line separators the values written contain, the will be
	 * escaped and the entire output
	 * of a record will be written into a single line of text. For example, '\n' and
	 * '\r' characters will be
	 * written as: {@code '\'+'n'} and {@code '\'+'r'}.
	 *
	 * If line joining is enabled, the {@link TsvWriter} will convert line new line
	 * characters into sequences
	 * containing the escape character, followed by characters '\n', '\r' or both.
	 * A new line of text will be generated for each line separator found in the
	 * value to be written, "marking" the end
	 * of each line with the escape character to indicate the record continues on
	 ...
	 * Values containing line endings should be escaped and the line separator
	 ...
	 * characters will be written as: {@code '\'+'\n'} and {@code '\'+'\r'}.
	 *
	 * @param lineJoiningEnabled a flag indicating whether or not to enable line
	 *                           joining.
	 */

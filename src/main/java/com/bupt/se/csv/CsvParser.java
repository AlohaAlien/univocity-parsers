...
import static com.bupt.se.csv.UnescapedQuoteHandling.*;
...
public final class CsvParser extends AbstractParser<CsvParserSettings> {
	...
	/**
	 * The CsvParser supports all settings provided by {@link CsvParserSettings},
		...
		while (ch != newLine) {
			...
			if (ch == delimiter || ch == newLine) {
				...
				if (ch == quote) {
					...
					if (len == 0) {
						String value = input.getQuotedString(quote, quoteEscape, escapeEscape, maxColumnLength,
								delimiter, newLine, keepQuotes, keepEscape, trimQuotedLeading, trimQuotedTrailing);
						if (value != null) {
		...
		switch (quoteHandling) {
			...
			case RAISE_ERROR:
				throw new TextParsingException(context, "Unescaped quote character '" + quote
						+ "' inside " + (quoted ? "quoted" : "")
						+ " value of CSV field. To allow unescaped quotes, set 'parseUnescapedQuotes' to 'true' in the CSV parser settings. Cannot parse CSV input.");
		}
	}
	...
	private void parseQuotedValue() {
		if (prev != '\0' && parseUnescapedQuotesUntilDelimiter) {
			...
			// handles whitespaces after quoted value: whitespaces are ignored. Content
			// after whitespaces may be parsed if 'parseUnescapedQuotes' is enabled.
			...
			if (ch != delimiter && ch != newLine) {
				throw new TextParsingException(context,
						"Unexpected character '" + ch + "' following quoted value of CSV field. Expecting '" + delimiter
	...
	/**
	 * Returns the CSV format detected when one of the following settings is
	 * enabled:
	 * <ul>
	 ...
	 * </ul>
	 *
	 * The detected format will be available once the parsing process is initialized
	 * (i.e. when {@link AbstractParser#beginParsing(Reader) runs}.
	 *
	 * @return the detected CSV format, or {@code null} if no detection has been
	 *         enabled or if the parsing process has not been started yet.
	 */
	...
	private void skipWhitespace() {
		while (ch <= ' ' && match < multiDelimiter.length && ch != newLine && ch != quote

 ...
 * {@link Format}, the CSV format defines:
 *
 * <ul>
 * <li><b>delimiter <i>(defaults to ',')</i>: </b> the field delimiter
 * character. Used to separate individual fields in a CSV record (where the
 * record is usually a line of text with multiple fields).
 * <p>
 * e.g. the value a , b is parsed as [ a ][ b ]</li>
 * <li><b>quote <i>(defaults to '"')</i>:</b> character used for escaping values
 * where the field delimiter is part of the value.
 * <p>
 * e.g. the value " a , b " is parsed as [ a , b ] (instead of [ a ][ b ]</li>
 * <li><b>quoteEscape <i>(defaults to '"')</i>:</b> character used for escaping
 * the quote character inside an already quoted value
 * <p>
 * e.g. the value " "" a , b "" " is parsed as [ " a , b " ] (instead of [ " a
 * ][ b " ] or [ "" a , b "" ])</li>
 * <li><b>charToEscapeQuoteEscaping <i>(defaults to '\0' - undefined)</i>:</b>
 * character used for escaping the escape for the quote character
 * <p>
 * e.g. if the quoteEscape and charToEscapeQuoteEscaping are set to '\', the
 * value " \\\" a , b \\\" " is parsed as [ \" a , b \" ]</li>
 * </ul>
 *
...
public class CsvFormat extends Format {
	...
	/**
	 * Identifies whether or not a given character is used for escaping values where
	 * the field delimiter is part of the value
	 *
	...
	/**
	 * Returns the character used for escaping quotes inside an already quoted
	 * value. Defaults to '"'
	 *
	...
	public char getDelimiter() {
		if (delimiter.length() > 1) {
			throw new UnsupportedOperationException(
					"Delimiter '" + delimiter + "' has more than one character. Use method getDelimiterString()");
		}
	 ...
	 * @param ch the character to be verified
	 *
	 * @return true if the given character is the field delimiter character, false
	 *         otherwise
	 */
	public boolean isDelimiter(char ch) {
		if (delimiter.length() > 1) {
			throw new UnsupportedOperationException(
					"Delimiter '" + delimiter + "' has more than one character. Use method isDelimiter(String)");
		}
	 ...
	 *
	 * @return true if the given sequence is the field delimiter character sequence,
	 *         false otherwise
	 */
	...
	/**
	 * Returns the character used to escape the character used for escaping quotes
	 * defined by {@link #getQuoteEscape()}.
	 * For example, if the quote escape is set to '\', and the quoted value ends
	 ...
	 * Then:
	 * <ul>
	 * <li>If the character to escape the '\' is undefined, the record won't be
	 * parsed. The parser will read characters: [a],[\],["],[,],[ ],[b] and throw an
	 * error because it cannot find a closing quote</li>
	 * <li>If the character to escape the '\' is defined as '\', the record will be
	 * read with 2 values: [a\] and [b]</li>
	 * </ul>
	 * Defaults to '\0' (undefined)
	 *
	 * @return the character to escape the character used for escaping quotes
	 *         defined
	 */
	 ...
	 * Defaults to '\0' (undefined)
	 *
	 * @param charToEscapeQuoteEscaping the character to escape the character used
	 *                                  for escaping quotes defined
	 */
	...
	/**
	 * Identifies whether or not a given character is used to escape the character
	 * used for escaping quotes defined by {@link #getQuoteEscape()}.
	 *
	 * @param ch the character to be verified
	 *
	 * @return true if the given character is used to escape the quote escape

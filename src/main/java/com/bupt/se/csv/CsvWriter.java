...
public class CsvWriter extends AbstractWriter<CsvWriterSettings> {
	...
	/**
	 * The CsvWriter supports all settings provided by {@link CsvWriterSettings},
	 ...
	 * and requires this configuration to be properly initialized.
	 *
	 * @param writer   the output resource that will receive CSV records produced by
	 *                 this class.
	 * @param settings the CSV writer configuration
	 */
	 ...
	 * and requires this configuration to be properly initialized.
	 *
	 * @param file     the output file that will receive CSV records produced by
	 *                 this class.
	 * @param settings the CSV writer configuration
	 */
	 ...
	 * and requires this configuration to be properly initialized.
	 *
	 * @param output   the output stream that will be written with the CSV records
	...
	protected void processRow(Object[] row) {
		...
		for (int i = 0; i < row.length; i++) {
			...
			boolean isElementQuoted = append(i, quoteOn && (quoteAllFields || quotedColumns.contains(i)), allowTrim,
					nextElement) && quoteOn;
	...
	private boolean quoteElement(int start, String element) {
		...
		if (multiDelimiter == null) {
			...
			if (maxTrigger == 0) {
				...
				for (int i = start; i < length; i++) {
					char nextChar = element.charAt(i);
					if ((nextChar == multiDelimiter[0] && matchMultiDelimiter(element, i + 1))
							|| nextChar < maxTrigger && quotationTriggers[nextChar]) {
						return true;
					}
				}
			}
	...
	private boolean append(int columnIndex, boolean isElementQuoted, boolean allowTrim, String element) {
		...
		final int length = element.length();
		if (start < length
				&& (element.charAt(start) == quoteChar || columnIndex == 0 && element.charAt(0) == comment)) {
		...
		if (multiDelimiter == null) {
			for (; i < length; i++) {
				ch = element.charAt(i);
				if (ch == quoteChar || ch == delimiter || ch == escapeChar
						|| (ch < maxTrigger && quotationTriggers[ch])) {
					...
					if (ch == quoteChar || ch == escapeChar) {
						if (quoteElement(i, element)) {
							...
							if (allowTrim && ignoreTrailing && element.charAt(length - 1) <= ' '
									&& whitespaceRangeStart < element.charAt(length - 1)) {
			...
			for (; i < length; i++) {
				ch = element.charAt(i);
				if (ch == quoteChar || (ch == multiDelimiter[0] && matchMultiDelimiter(element, i + 1))
						|| ch == escapeChar || (ch < maxTrigger && quotationTriggers[ch])) {
					appender.append(element, start, i);
					start = i + 1;
					...
					if (ch == quoteChar || ch == escapeChar) {
						...
						appender.append(escapeEscape);
					} else if ((ch == multiDelimiter[0] && matchMultiDelimiter(element, i + 1))
							|| ch < maxTrigger && quotationTriggers[ch]) {
						appendQuoted(i, allowTrim, element);
						return true;
					}

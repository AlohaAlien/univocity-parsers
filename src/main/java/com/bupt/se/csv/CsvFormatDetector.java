...
/**
 * An {@link InputAnalysisProcess} to detect column delimiters, quotes and quote
 * escapes in a CSV input.
 *
...
public abstract class CsvFormatDetector implements InputAnalysisProcess {
	 ...
	 *
	 * @param maxRowSamples        the number of row samples to collect before
	 *                             analyzing the statistics
	 * @param settings             the configuration provided by the user with
	 *                             potential defaults in case the detection is
	 *                             unable to discover the proper column
	...
	public void execute(char[] characters, int length) {
		...
		for (i = 0; i < length; i++) {
				...
				if (inQuote == ch) { // closing quotes (potentially)
					...
					if (i + 1 < length) {
						...
						if (Character.isLetterOrDigit(next) || (next <= ' ' && whitespaceRangeStart < next
								&& next != '\n' && next != '\r' && next != '\t')) { // no special characters after
																					// quote, might be escaping
			...
			} else if ((ch == '\r' || ch == '\n' || ch == normalizedNewLine) && symbols.size() > 0) { // got a newline
																										// and collected
																										// some symbols?
																										// Good!
				afterNewLine = true;
		...
		if (length >= characters.length && i >= length && symbolsPerRow.size() > 1) { // if got to the end of the
																						// buffer, discard last row.
		...
		for (Map<Character, Integer> prev : symbolsPerRow) {
			for (Map<Character, Integer> current : symbolsPerRow) {
				for (Character symbol : allSymbols) {
					...
					if (previousCount == null && currentCount == null) { // got a symbol that does not appear in all
																			// rows? Discard it.
						toRemove.add(symbol);
					}
					...
					increment(sums, symbol, Math.abs(previousCount - currentCount)); // we expect to always get 0 or
																						// close to 0 here, so the
																						// symbol occurs in all rows
				}
			}
		}

		if (toRemove.size() == sums.size()) { // will discard all symbols. Stick with the symbols that showed up more
	 ...
	 * @param map         the map of characters and their numbers
	 * @param defaultChar the default character to return in case the map is empty
	 * @param min         a flag indicating whether to return the character
	 *                    associated with the lowest number in the map.
	 *                    If {@code false} then the character associated with the
	 *                    highest number found will be returned.
	 *

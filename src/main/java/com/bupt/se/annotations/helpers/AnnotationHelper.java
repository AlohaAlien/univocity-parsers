 ...
 ******************************************************************************/
package com.bupt.se.annotations.helpers;
...
public class AnnotationHelper {
	 ...
	 * {@link Parsed#defaultNullRead() and Parsed#defaultNullWrite()}
	 *
	 * @param defaultValue The string returned by {@link Parsed#defaultNullRead()
	...
	/**
	 * Identifies the proper conversion for a given type and an annotation from the
	 * package {@link com.bupt.se.annotations}
	 *
	 * @param classType  the type to have conversions applied to
	 * @param annotation the annotation from {@link com.bupt.se.annotations} that
	 *                   identifies a {@link Conversion} instance.
	 *
	...
	public static EnumConversion createDefaultEnumConversion(Class fieldType, String nullRead, String nullWrite) {
		...
		return new EnumConversion(fieldType, nullReadValue, nullWrite, null, EnumSelector.NAME, EnumSelector.ORDINAL,
				EnumSelector.STRING);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static Conversion getConversion(Class fieldType, AnnotatedElement target, Annotation annotation,
			String nullRead, String nullWrite) {
		try {
			...
			if (annType == NullString.class) {
				...
				boolean allowBlanks = AnnotationRegistry.getValue(target, validate, "allowBlanks",
						validate.allowBlanks());
				...
				if (!fieldType.isEnum()) {
					if (target == null) {
						throw new IllegalStateException("Invalid " + EnumOptions.class.getName()
								+ " instance for converting class " + fieldType.getName() + ". Not an enum type.");
					} else {
						throw new IllegalStateException("Invalid " + EnumOptions.class.getName() + " annotation on "
								+ describeElement(target) + ". Attribute must be an enum type.");
					}
				}
				EnumOptions enumOptions = ((EnumOptions) annotation);
				String customElement = AnnotationRegistry.getValue(target, enumOptions, "customElement",
						enumOptions.customElement());
				...
				Enum nullReadValue = nullRead == null ? null : Enum.valueOf(fieldType, nullRead);
				EnumSelector[] selectors = AnnotationRegistry.getValue(target, enumOptions, "selectors",
						enumOptions.selectors());
				...
				if (fieldType != boolean.class && fieldType != Boolean.class) {
					if (target == null) {
						throw new DataProcessingException("Invalid  usage of " + BooleanString.class.getName()
								+ ". Got type " + fieldType.getName() + " instead of boolean.");
					} else {
						throw new DataProcessingException("Invalid annotation: " + describeElement(target)
								+ " has type " + fieldType.getName() + " instead of boolean.");
					}
				}
				BooleanString boolString = ((BooleanString) annotation);
				String[] falseStrings = AnnotationRegistry.getValue(target, boolString, "falseStrings",
						boolString.falseStrings());
				String[] trueStrings = AnnotationRegistry.getValue(target, boolString, "trueStrings",
						boolString.trueStrings());
				Boolean valueForNull = nullRead == null ? null
						: BooleanConversion.getBoolean(nullRead, trueStrings, falseStrings);
				...
				if (fieldType == BigDecimal.class) {
					...
					conversion = Conversions.formatToBigDecimal(defaultForNull, nullWrite, formats);
						...
						|| (fieldType.isPrimitive()) && fieldType != boolean.class && fieldType != char.class) {
				...
				if (conversion != null) {
					if (options.length > 0) {
						if (conversion instanceof FormattedConversion) {
							...
							throw new DataProcessingException("Options '" + Arrays.toString(options)
									+ "' not supported by conversion of type '" + conversion.getClass()
									+ "'. It must implement " + FormattedConversion.class);
						}
					}
					return conversion;
				}
				...
				Class conversionClass = AnnotationRegistry.getValue(target, convert, "conversionClass",
			...
			if (target == null) {
				throw new DataProcessingException(
						"Unexpected error identifying conversions to apply over type " + fieldType, ex);
			} else {
				throw new DataProcessingException(
						"Unexpected error identifying conversions to apply over " + describeElement(target), ex);
			}
		}
	}
	...
	private static Locale extractLocale(String[] options) {
		...
		if (locale != null) {
			...
			for (++j; j < locale.length() && Character.isLetterOrDigit((ch = locale.charAt(j))); j++, appender
					.append(ch))
				;
	...
	public static <T> T newInstance(Class parent, Class<T> type, String[] args) {
		if (!parent.isAssignableFrom(type)) {
			throw new DataProcessingException("Not a valid " + parent.getSimpleName() + " class: '"
		...
		try {
			...
			if (args.length == 0) {
				try {
					...
					throw new DataProcessingException("Unexpected error instantiating custom " + parent.getSimpleName()
							+ " class '" + type.getSimpleName() + "' (" + type.getName() + ')', e);
				}
			}
			throw new DataProcessingException("Could not find a public constructor with a String[] parameter in custom "
					+ parent.getSimpleName() + " class '" + type.getSimpleName() + "' (" + type.getName() + ')', e);
		} catch (Exception e) {
			throw new DataProcessingException("Unexpected error instantiating custom " + parent.getSimpleName()
					+ " class '" + type.getSimpleName() + "' (" + type.getName() + ')', e);
		}
	 ...
	 * @param parsed    the {@link Parsed} annotation from
	 *                  {@link com.bupt.se.annotations}.
	 *
	...
	public static Conversion getDefaultConversion(Class fieldType, AnnotatedElement target, Parsed parsed) {
		...
		if (fieldType == Boolean.class || fieldType == boolean.class) {
			...
			if (nullRead != null && nullRead.length() > 1) {
				throw new DataProcessingException("Invalid default value for character '" + nullRead
						+ "'. It should contain one character only.");
			}
	...
	/**
	 * Returns the default {@link Conversion} that should be applied to the field
	 * based on its type.
	 *
	 * @param target The field or method whose values must be converted from a given
	 *               parsed String.
	 *
	...
	/**
	 * Applied the configuration of a formatter object ({@link SimpleDateFormat},
	 * {@link NumberFormat} and others).
	 *
	 * @param formatter           the formatter instance
	 * @param propertiesAndValues a sequence of key-value pairs, where the key is a
	 *                            property of the formatter
	 *                            object to be set to the following value via
	 *                            reflection
	 */
	public static void applyFormatSettings(Object formatter, String[] propertiesAndValues) {
		...
		for (String setting : propertiesAndValues) {
			...
			if (pair.length != 2) {
				throw new DataProcessingException(
						"Illegal format setting '" + setting + "' among: " + Arrays.toString(propertiesAndValues));
			}
		...
		try {
			for (PropertyWrapper property : BeanHelper.getPropertyDescriptors(formatter.getClass())) {
				...
				if ("decimalFormatSymbols".equals(property.getName())) {
					...
					try {
						for (PropertyWrapper prop : BeanHelper
								.getPropertyDescriptors(modifiedDecimalSymbols.getClass())) {
						...
						if (modified) {
							...
							if (writeMethod != null) {
								...
								throw new IllegalStateException(
										"No write method defined for property " + property.getName());
							}
						}
					} catch (Throwable ex) {
						throw new DataProcessingException(
								"Error trying to configure decimal symbols of formatter '" + formatter.getClass() + '.',
								ex);
					}
				}
			}
		...
		if (!values.isEmpty()) {
			throw new DataProcessingException(
					"Cannot find properties in formatter of type '" + formatter.getClass() + "': " + values);
		}
	}
	...
	private static void invokeSetter(Object formatter, PropertyWrapper property, final String value) {
		Method writeMethod = property.getWriteMethod();
		if (writeMethod == null) {
			DataProcessingException exception = new DataProcessingException("Cannot set property '" + property.getName()
					+ "' of formatter '" + formatter.getClass() + "' to '{value}'. No setter defined");
		...
		if (parameterValue == null) {
			DataProcessingException exception = new DataProcessingException(
					"Cannot set property '" + property.getName() + "' of formatter '" + formatter.getClass()
							+ ". Cannot convert '{value}' to instance of " + parameterType);
			exception.setValue(value);
			throw exception;
		}
		...
		try {
			...
			DataProcessingException exception = new DataProcessingException(
					"Error setting property '" + property.getName() + "' of formatter '" + formatter.getClass()
							+ ", with '{parameterValue}' (converted from '{value}')",
					e);
	...
	/**
	 * Runs through all annotations of a given class to identify whether all
	 ...
	 *
	 * @return {@code true} if every field and method annotated with {@link Parsed}
	 *         in the given class maps to an index, otherwise {@code false}.
	 */
	 ...
	 *
	 * @return {@code true} if every field and method annotated with {@link Parsed}
	 *         in the given class maps to a header name, otherwise {@code false}.
	 */
	...
	/**
	 * Runs through all {@link Parsed} annotations of a given class to identify all
	 * indexes associated with its fields
	 *
	 * @param beanClass a class whose {@link Parsed} annotations will be processed.
	 * @param filter    filter to apply over annotated methods when the class is
	 *                  being used for reading data from beans (to write values to
	 *                  an output)
	 *                  or when writing values into beans (while parsing). It is
	 *                  used to choose either a "get" or a "set"
	 *                  method annotated with {@link Parsed}, when both methods
	...
	public static Integer[] getSelectedIndexes(Class<?> beanClass, MethodFilter filter) {
		...
		for (TransformedHeader header : getFieldSequence(beanClass, true, null, filter)) {
			...
			if (index != -1) {
				if (filter == MethodFilter.ONLY_GETTERS && indexes.contains(index)) { // allows the same column to be
																						// mapped to multiple fields
																						// when parsing, but not when
																						// writing.
					throw new IllegalArgumentException("Duplicate field index '" + index + "' found in attribute '"
							+ header.getTargetName() + "' of class " + beanClass.getName());
				}
				indexes.add(index);
			}
		}
	...
	/**
	 * Runs through all {@link Parsed} annotations of a given class to identify all
	 * header names associated with its fields
	 *
	 * @param beanClass a class whose {@link Parsed} annotations will be processed.
	 * @param filter    a filter to exclude annotated methods that won't be used for
	 *                  parsing or writing
	 *
	...
	public static String[] deriveHeaderNamesFromFields(Class<?> beanClass, MethodFilter filter) {
		...
		for (TransformedHeader field : sequence) {
			if (field == null) {
				return ArgumentUtils.EMPTY_STRING_ARRAY; // some field has an index that goes beyond list of header
															// names, can't derive.
			}
	 ...
	 * @return the annotation of the given class or its most immediate parent, or
	 ...
	 * @param beanClass the class whose hierarchy will be searched
	 *
	 * @return the {@link Headers} annotation of the given class or its most
	 *         immediate parent, or {@code null} if not found.
	 */
	...
	public static Class<?> getType(AnnotatedElement element) {
		...
		if (params.length == 1) {
			...
			throw new IllegalArgumentException(
					"Method " + describeElement(element) + " cannot have multiple parameters");
		}
		...
		throw new IllegalArgumentException(
				"Method " + describeElement(element) + " must return a value if it has no input parameter");
	}
	...
	private static void processAnnotations(AnnotatedElement element, boolean processNested, List<Integer> indexes,
			List<TransformedHeader> tmp, Map<AnnotatedElement, List<TransformedHeader>> nestedReplacements,
			HeaderTransformer transformer, MethodFilter filter) {
		Parsed annotation = findAnnotation(element, Parsed.class);
		if (annotation != null) {
			TransformedHeader header = new TransformedHeader(element, transformer);

			if (filter == MethodFilter.ONLY_GETTERS && header.getHeaderIndex() >= 0
					&& indexes.contains(header.getHeaderIndex())) {
				// allows the same column to be mapped to multiple fields when parsing, but not
				// when writing.
				throw new IllegalArgumentException(
						"Duplicate field index '" + header.getHeaderIndex() + "' found in " + describeElement(element));
			}
			tmp.add(header);
		...
		if (processNested) {
			...
			if (nested != null) {
				...
				Class<? extends HeaderTransformer> transformerType = AnnotationRegistry.getValue(element, nested,
						"headerTransformer", nested.headerTransformer());
				if (transformerType != HeaderTransformer.class) {
					String[] args = AnnotationRegistry.getValue(element, nested, "args", nested.args());
					HeaderTransformer innerTransformer = AnnotationHelper.newInstance(HeaderTransformer.class,
							transformerType, args);
					nestedReplacements.put(element,
							getFieldSequence(nestedBeanType, true, indexes, innerTransformer, filter));
				} else {
					nestedReplacements.put(element,
							getFieldSequence(nestedBeanType, true, indexes, transformer, filter));
				}
			}
		}
	}
	...
	/**
	 * Returns a list of fields with {@link Parsed} annotations in the sequence they
	 * should be processed for parsing
	 * or writing. The sequence is ordered taking into account their original order
	 * in the annotated class, unless
	 * {@link Parsed#index()} is set to a non-negative number.
	 *
	 * @param beanClass     the class whose field sequence will be returned.
	 * @param processNested flag indicating whether {@link Nested} annotations
	 *                      should be processed
	 * @param transformer   a {@link HeaderTransformer} instance to be used for
	 *                      transforming headers of a given {@link Nested}
	 *                      attribute.
	 * @param filter        filter to apply over annotated methods when the class is
	 *                      being used for reading data from beans (to write values
	 *                      to an output)
	 *                      or when writing values into beans (while parsing). It is
	 *                      used to choose either a "get" or a "set"
	 *                      method annotated with {@link Parsed}, when both methods
	 ...
	 */
	public static List<TransformedHeader> getFieldSequence(Class beanClass, boolean processNested,
			HeaderTransformer transformer, MethodFilter filter) {
		List<Integer> indexes = new ArrayList<Integer>();
	...
	private static List<TransformedHeader> getFieldSequence(Class beanClass, boolean processNested,
			List<Integer> indexes, HeaderTransformer transformer, MethodFilter filter) {
	...
	/**
	 * Returns all methods available from a given class that have a specific
	 * annotation.
	 *
	 * @param beanClass      a class whose methods will be returned.
	 * @param filter         filter to apply over annotated methods when the class
	 *                       is being used for reading data from beans (to write
	 *                       values to an output)
	 *                       or when writing values into beans (while parsing). It
	 *                       is used to choose either a "get" or a "set"
	 *                       method annotated with {@link Parsed}, when both methods
	 *                       target the same field.
	 ...
	 */
	public static <A extends Annotation> List<Method> getAnnotatedMethods(Class<?> beanClass, MethodFilter filter,
		...
		do {
			...
			outer: for (Method method : declared) {
				if (!method.isSynthetic() && annotationType == NO_ANNOTATIONS.class) {
					...
					for (Annotation annotation : annotations) {
						if ((annotationType == null && isCustomAnnotation(annotation))
	...
	/**
	 * Searches for an annotation of a given type that's been applied to an element
	 * either directly (as a regular annotation)
	 * or indirectly (as a meta-annotations, i.e. an annotation that has
	 * annotations).
	 *
	 ...
	 *
	 * @return the annotation associated with the given element, or {@code null} if
	 *         not found.
	 */
	public synchronized static <A extends Annotation> A findAnnotation(AnnotatedElement annotatedElement,
			Class<A> annotationType) {
			...
			for (Method method : parent.annotationType().getDeclaredMethods()) {
				...
				if (copy != null) {
					...
					if (targetClass == target.annotationType()) {
						...
						A ann = (A) findAnnotation(annotatedElement, targetClass, new HashSet<Annotation>(),
						...
						if (ann != null) {
							...
							throw new IllegalStateException("Can't process @Copy annotation on '" + method + "'. " +
									"Annotation '" + targetClass.getName() + "' not used in "
									+ parent.annotationType().getName() + ". Unable to process field "
									+ annotatedElement + "'");
						}
					}
	...
	private static <A> A findAnnotation(AnnotatedElement annotatedElement, Class<A> annotationType,
			Set<Annotation> visited, Stack<Annotation> path) {
	...
	/**
	 * Returns all annotations applied to an element, excluding the ones not in a
	 * given package.
	 *
	 * @param annotatedElement the element (method, field, etc) whose annotations
	 *                         will be extracted
	 * @param aPackage         the package of the annotations that should be
	 *                         returned
	 *
	 * @return the list of annotation elements applied to the given element, that
	 *         are also members of the given package.
	 */
	...
	private static void findAllAnnotationsInPackage(AnnotatedElement annotatedElement, Package aPackage,
			ArrayList<? super Annotation> found, Set<Annotation> visited) {
		Annotation[] declaredAnnotations = annotatedElement.getDeclaredAnnotations();
	 ...
	 *
	 * @return the default value for the given primitive type, or {@code null} if

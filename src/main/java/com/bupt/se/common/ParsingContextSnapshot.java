...
/**
 * A snapshot of a {@link ParsingContext} which retains copies of variable
 * attributes of a given {@link ParsingContext} to
 * store the state of the parsing process at a given point in time. All runtime
 * operations such as {@link #stop()}
...
public class ParsingContextSnapshot extends ParsingContextWrapper {
	 ...
	 *
	 * @param context the context object whose variable attributes will be copied
	 *                over.
	 */
	public ParsingContextSnapshot(ParsingContext context) {
		...
		comments = context.comments() == Collections.EMPTY_MAP ? Collections.<Long, String>emptyMap()
				: Collections.unmodifiableMap(context.comments());

...
/**
 * The base class for {@link RowProcessor} and {@link RowWriterProcessor}
 * implementations that support value conversions provided by {@link Conversion}
 * instances.
 *
...
public abstract class DefaultConversionProcessor implements ConversionProcessor {
	 ...
	 * {@link DefaultConversionProcessor#convertFields(Conversion...)},
	 * {@link DefaultConversionProcessor#convertIndexes(Conversion...)} and
	 * {@link DefaultConversionProcessor#convertAll(Conversion...)}, for every field
	 * in the given row.
	 *
	 * <p>
	 * Each field will be transformed using the {@link Conversion#execute(Object)}
	 * method.
	 * <p>
	 * In general the conversions will process a String and convert it to some
	 * object value (such as booleans, dates, etc).
	 *
	 * @param row     the parsed record with its individual records as extracted
	 *                from the original input.
	 * @param context the current state of the parsing process.
	 *
	 * @return an row of Object instances containing the values obtained after the
	 *         execution of all conversions.
	 *         <p>
	 *         Fields that do not have any conversion defined will just be copied to
	 *         the object array into their original positions.
	 */
	 ...
	 *
	 * <p>
	 * Each field will be transformed using the {@link Conversion#revert(Object)}
	 * method.
	 * <p>
	 * In general the conversions will process an Object (such as a Boolean, Date,
	 * etc), and convert it to a String representation.
	 *
	 * @param executeInReverseOrder flag to indicate whether the conversion sequence
	 *                              should be executed in the reverse order of its
	 *                              declaration.
	 * @param row                   the row of objects that will be converted
	 * @param headers               All field names used to produce records in a
	 *                              given destination. May be null if no headers
	 *                              have been defined in
	 *                              {@link CommonSettings#getHeaders()}
	 * @param indexesToWrite        The indexes of the headers that are actually
	 *                              being written. May be null if no fields have
	 *                              been selected using
	 *                              {@link CommonSettings#selectFields(String...)}
	 *                              or
	 *                              {@link CommonSettings#selectIndexes(Integer...)}
	 *
	 * @return {@code true} if the the row should be discarded
	 */
	public final boolean reverseConversions(boolean executeInReverseOrder, Object[] row, NormalizedString[] headers,
			int[] indexesToWrite) {
		boolean keepRow = true;
		boolean[] convertedFlags = conversionsByType != null ? new boolean[row.length] : null;
		if (conversions != null) {
			if (!conversionsInitialized) {
				conversionsInitialized = true;
				conversions.prepareExecution(true,
						headers != null ? NormalizedString.toArray(headers) : new String[row.length]);
	...
	/**
	 * Handles an error that occurred when applying conversions over a value. If the
	 * user defined a
	 * {@link ProcessorErrorHandler} the user will receive the exception and is able
	 * to determine whether or not
	 * processing should continue, discarding the record. If the error handler is an
	 * instance of
	 * {@link RetryableErrorHandler}, the user can provide a default value to use in
	 * place of the one that could not
	 * be converted, and decide whether or not the record should be kept with the
	 * use of the
	 ...
	 * @param column the column if the given row, whose value could not be
	 *               converted. If negative, it's not possible to
	 *               keep the record.
	 *
	 * @return {@code true} if the error has been handled by the user and the record
	 *         can still be processed, otherwise
	...
	public final void convertType(Class<?> type, Conversion... conversions) {
		...
		ArgumentUtils.noNulls("Sequence of conversions to apply over data of type " + type.getSimpleName(),
				conversions);

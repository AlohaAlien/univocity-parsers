...
/**
 * A {@code NormalizedString} allows representing text in a normalized fashion.
 * Strings
 * with different character case or surrounding whitespace are considered the
 * same.
 *
 * Used to represent groups of fields, where users may refer to their names
 * using
 * different character cases or whitespaces.
 *
 * Where the character case or the surrounding space is relevant, the
 * {@code NormalizedString}
 * will have its {@link #isLiteral()} method return {@code true}, meaning the
 * exact
 * character case and surrounding whitespaces are required for matching it.
 *
 * Invoking {@link #valueOf(String)} with a {@code String} surrounded by single
 * quotes
 * will create a literal {@code NormalizedString}. Use
 * {@link #literalValueOf(String)}
 * to obtain the same {@code NormalizedString} without having to introduce
 * single quotes.
 *
 */
public final class NormalizedString implements Serializable, Comparable<NormalizedString>, CharSequence {
	...
	/**
	 * Compares a {@code NormalizedString} against a {@code String}
	 * lexicographically.
	 * 
	 * @param o a plain {@code String}
	 * @return the result of {@link String#compareTo(String)}. If this
	 *         {@code NormalizedString}
	 *         is a literal, the original argument string will be compared. If this
	 *         {@code NormalizedString}
	 *         is not a literal, the result will be from the comparison of the
	 *         normalized content of both strings
	 *         (i.e. surrounding whitespaces and character case differences will be
	 *         ignored).
	 */
	 ...
	 * other {@code String} or {@code NormalizedString} if they have the exact same
	 * content
	 ...
	 * including character case and surrounding whitespaces.
	 *
	 * If the input value is enclosed with single quotes, a literal
	 * {@code NormalizedString}
	 ...
	 * including character case and surrounding whitespaces.
	 *
	 * If the input string is enclosed with single quotes, a literal
	 * {@code NormalizedString}
	 * will be returned, as described in {@link #literalValueOf(String)}
	 *
	...
	/**
	 * Converts a {@code NormalizedString} back to its original {@code String}
	 * representation
	 * 
	 * @param string the normalized string
	 * @return the original string used to create the given normalized
	 *         representation.
	 */
	 ...
	 * ensuring
	 * no duplicate {@code NormalizedString} elements exist, even if their original
	 * {@code String}s
	 * are different.
	 *
	 * @param args the strings to convert to {@code NormalizedString}
	...
	/**
	 * Converts multiple plain strings into an {@code ArrayList} of
	 * {@code NormalizedString}.
	 *
	 * @param args the strings to convert to {@code NormalizedString}
	 * @return the {@code NormalizedString} representations of all input strings.
	...
	/**
	 * Converts multiple normalized strings into a {@code HashSet} of
	 * {@code String}.
	 *
	 * @param args the normalized strings to convert to {@code String}
	...
	/**
	 * Converts multiple plain strings into a {@code TreeSet} of
	 * {@code NormalizedString}.
	 *
	 * @param args the strings to convert to {@code NormalizedString}
	 * @return the {@code NormalizedString} representations of all input strings.
	...
	/**
	 * Converts multiple plain strings into a {@code HashSet} of
	 * {@code NormalizedString}.
	 *
	 * @param args the strings to convert to {@code NormalizedString}
	 * @return the {@code NormalizedString} representations of all input strings.
	...
	/**
	 * Converts multiple plain strings into a {@code LinkedHashSet} of
	 * {@code NormalizedString}.
	 *
	 * @param args the strings to convert to {@code NormalizedString}
	 * @return the {@code NormalizedString} representations of all input strings.
	...
	/**
	 * Converts multiple normalized strings into a {@code LinkedHashSet} of
	 * {@code String}.
	 *
	 * @param args the normalized strings to convert to {@code String}
	 * @return the original {@code String}s of all input normalized strings.
	...
	/**
	 * Returns the literal representation of this {@code NormalizedString}, meaning
	 * it will only match with
	 * other {@code String} or {@code NormalizedString} if they have the exact same
	 * content
	 * including character case and surrounding whitespaces.
	 *
	...
	/**
	 * Analyzes a group of NormalizedString to identify any instances whose
	 * normalized content will generate
	 * clashes. Any clashing entries will be converted to their literal counterparts
	 * (using {@link #toLiteral()}),
	 * making it possible to identify one from the other.
	 *
	 * @param strings a group of identifiers that may contain ambiguous entries if
	 *                their character case or surrounding whitespaces is not
	 *                considered.
	 *                This array will be modified.
	 *
	 * @return the input string array, with {@code NormalizedString} literals in the
	 *         positions where clashes would originally occur.
	 */
	...
	/**
	 * Analyzes a group of String to identify any instances whose normalized content
	 * will generate
	 * clashes. Any clashing entries will be converted to their literal counterparts
	 * (using {@link #toLiteral()}),
	 * making it possible to identify one from the other.
	 ...
	 *
	 *
	 * @return a {@code NormalizedString} array with literals in the positions where
	 *         clashes would originally occur.
	 */
	 ...
	 *                This array will be modified.
	 *
	 * @return {@code true} if any entry has been modified to be a literal,
	 ...
	 * making it possible to identify one from the other.
	 *
	 * @param strings              a group of identifiers that may contain ambiguous
	 *                             entries if their character case or surrounding
	 *                             whitespaces is not considered.
	 *                             This array will be modified.
	 *
	 * @param lowercaseIdentifiers flag indicating that identifiers are stored in
	 *                             lower case (for compatibility with databases).
	 *                             If a string has a uppercase character, it means
	 *                             it must become a literal.
	 * @param uppercaseIdentifiers flag indicating that identifiers are stored in
	 *                             upper case (for compatibility with databases).
	 *                             If a string has a lowercase character, it means
	 *                             it must become a literal.
	 *
	 * @return {@code true} if any entry has been modified to be a literal,
	 *         otherwise {@code false}
	 *
	 */
	public static boolean identifyLiterals(NormalizedString[] strings, boolean lowercaseIdentifiers,
	...
	private static boolean shouldBeLiteral(String string, boolean lowercaseIdentifiers, boolean uppercaseIdentifiers) {
		if (lowercaseIdentifiers || uppercaseIdentifiers) {
			for (int i = 0; i < string.length(); i++) {
				...
				if ((uppercaseIdentifiers && !Character.isUpperCase(ch))
						|| (lowercaseIdentifiers && !Character.isLowerCase(ch))) {
					return true;
				}
			}
		}
	...
	/**
	 * Returns the internal string cache to allow users to tweak its size limit or
	 * clear it when appropriate
	 * 
	 * @return the string cache used to store {@code NormalizedString} instances
	 *         associated with their original {@code String}.
	 */

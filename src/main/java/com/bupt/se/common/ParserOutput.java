 ...
 * {@link AbstractParser} and their values.
 *
 * It is solely responsible for deciding when:
 * <ul>
 * <li>parsed records should be reordered according to the fields selected in
 * {@link CommonSettings}</li>
 * <li>characters and values parsed in {@link AbstractParser#parseRecord()}
 * should be retained or discarded</li>
 * <li>input headers should be loaded from the records parsed in
 * {@link AbstractParser#parseRecord()} or from
 * {@link CommonSettings#getHeaders()}</li>
 * </ul>
 *
 * Implementations of this class are made available to concrete parser
 * implementations of {@link AbstractParser}.
 *
...
public class ParserOutput {
	 ...
	 * Calls to {@link ParserOutput#valueParsed} and
	 * {@link ParserOutput#emptyParsed} will increase the column count.
	...
	/**
	 * <p>
	 * Stores (shared) references to {@link CharAppender} for each potential column
	 * (as given by {@link CommonSettings#getMaxColumns()}).
	 * <p>
	 * Fields that are not selected will receive an instance of
	 * {@link NoopCharAppender} so all parser calls in
	 * {@link AbstractParser#parseRecord()} to {@link ParserOutput#appender} will do
	 * nothing.
	 * <p>
	 * Selected fields (given by {@link CommonParserSettings}) will receive a
	 * functional {@link CharAppender}.
	 */
	...
	/**
	 * <p>
	 * The appender available to parsers for accumulating characters read from the
	 * input.
	 * <p>
	 * This attribute is assigned to different instances of CharAppender during
	 * parsing process, namely,
	 ...
	 * irrespective of any user-provided headers in
	 * {@link CommonParserSettings#getHeaders()}
	 */
				...
				if (header != null && !header.isLiteral()) {
					if (settings.getIgnoreLeadingWhitespaces()) {
						if (settings.getIgnoreTrailingWhitespaces()) {
							...
							headers[i] = NormalizedString
									.valueOf(ArgumentUtils.trim(headers[i].toString(), true, false));
						}
	 ...
	 *
	 * @param values a sequence of values that represent the headers of the input.
	 *               This can be either a parsed record or the headers as defined in
	...
	/**
	 * Returns the sequence of values that represent the headers each field in the
	 * input. This can be either a parsed record or the headers as defined in
	 * {@link CommonSettings#getHeaders()}
	 *
	 ...
	 * {@link CommonSettings}. Null if no fields were selected.
	 *
	 * @return the selected indexes of all fields as defined in
	 *         {@link CommonSettings}. Null if no fields were selected.
	 */
	 ...
	 * {@link CommonSettings}) are being reordered.
	 *
	 * @return
	 *         <p>
	 *         false if no fields were selected or column reordering has been
	 *         disabled in {@link CommonParserSettings#isColumnReorderingEnabled()}
	 *         <p>
	 *         true if fields were selected and column reordering has been enabled
	 *         in {@link CommonParserSettings#isColumnReorderingEnabled()}
	 */
	...
	/**
	 * Adds a nullValue (as specified in {@link CommonSettings#getNullValue()}) to
	 * the output and prepares the next position in the record to receive more
	 * values.
	 */
	...
	/**
	 * Adds the accumulated value in the appender object to the output and prepares
	 * the next position in the record to receive more values.
	 */
	...
	/**
	 * Adds a value processed externally to the output and prepares the next
	 * position in the record to receive more values
	 *
	...
	/**
	 * Returns the current record index. The number returned here reflects the
	 * number of actually parsed and valid records sent to the output of
	 * {@link ParserOutput#rowParsed}.
	 *

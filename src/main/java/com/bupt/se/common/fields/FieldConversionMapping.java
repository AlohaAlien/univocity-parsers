 ...
 ******************************************************************************/
package com.bupt.se.common.fields;
...
/**
 * A class for mapping field selections to sequences of {@link Conversion}
 * objects
 *
...
public class FieldConversionMapping implements Cloneable {
	...
	/**
	 * This list contains the sequence of conversions applied to sets of fields over
	 * multiple calls.
	 * <p>
	 * It is shared by {@link FieldConversionMapping#fieldNameConversionMapping},
	 * {@link FieldConversionMapping#fieldIndexConversionMapping} and
	 * {@link FieldConversionMapping#convertAllMapping}.
	 * <p>
	 * Every time the user associates a sequence of conversions to a field,
	 * conversionSequence list will receive the FieldSelector.
	 */
	private List<FieldSelector> conversionSequence = new ArrayList<FieldSelector>();

	private AbstractConversionMapping<String> fieldNameConversionMapping = new AbstractConversionMapping<String>(
			conversionSequence) {
		@Override
	...
	private AbstractConversionMapping<Integer> fieldIndexConversionMapping = new AbstractConversionMapping<Integer>(
			conversionSequence) {
		@Override
		protected FieldSelector newFieldSelector() {
	...
	@SuppressWarnings("rawtypes")
	private AbstractConversionMapping<Enum> fieldEnumConversionMapping = new AbstractConversionMapping<Enum>(
			conversionSequence) {
		@Override
		protected FieldSelector newFieldSelector() {
	...
	private AbstractConversionMapping<Integer> convertAllMapping = new AbstractConversionMapping<Integer>(
			conversionSequence) {
		@Override
		protected FieldSelector newFieldSelector() {
	 ...
	 * {@link FieldConversionMapping#prepareExecution(boolean, String[])} is
	 * invoked.
	 */
	...
	/**
	 * Prepares the conversions registered in this object to be executed against a
	 * given sequence of fields
	 *
	 * @param writing flag indicating whether a writing process is being
	 *                initialized.
	 * @param values  The field sequence that identifies how records will be
	 *                organized.
	 *                <p>
	 *                This is generally the sequence of headers in a record, but it
	 *                might be just the first parsed row from a given input (as
	 *                field selection by index is allowed).
	 */
	public void prepareExecution(boolean writing, String[] values) {
		if (fieldNameConversionMapping.isEmpty() && fieldEnumConversionMapping.isEmpty()
				&& fieldIndexConversionMapping.isEmpty() && convertAllMapping.isEmpty()) {
			return;
		}
		...
		// Note this property is shared across all conversion mappings. This is required
		// so
	...
	/**
	 * Applies a sequence of conversions on a selection of enumerations that
	 * represent fields
	 *
	 * @param conversions the sequence of conversions to be applied
	 *
	...
	/**
	 * Applies a sequence of conversions associated with an Object value at a given
	 * index in a record.
	 *
	 * @param executeInReverseOrder flag to indicate whether or not the conversion
	 *                              sequence must be executed in reverse order
	 * @param index                 The index of parsed value in a record
	 * @param value                 The value in a record
	 * @param convertedFlags        an array of flags that indicate whether a
	 *                              conversion occurred. Used to determine whether
	 *                              or not a default conversion by type (specified
	 *                              with
	 *                              {@link ConversionProcessor#convertType(Class, Conversion[])})
	 *                              should be applied.
	 *
	 * @return the Object resulting from a sequence of conversions against the
	 *         original value.
	 */
	...
	public Object reverseConversions(boolean executeInReverseOrder, int index, Object value, boolean[] convertedFlags) {
		...
		if (conversions != null) {
			...
			try {
				...
				if (conversion != null) {
					exception = new DataProcessingException(
							"Error converting value '{value}' using conversion " + conversion.getClass().getName(), ex);
				} else {
	...
	/**
	 * Applies a sequence of conversions associated with a String value parsed from
	 * a given index.
	 *
	 * @param index          The index of parsed value in a record
	 * @param stringValue    The parsed value in a record
	 * @param convertedFlags an array of flags that indicate whether a conversion
	 *                       occurred. Used to determine whether
	 *                       or not a default conversion by type (specified with
	 *                       {@link ConversionProcessor#convertType(Class, Conversion[])})
	 *                       should be applied.
	 *
	 * @return the Object produced by a sequence of conversions against the original
	 *         String value.
	 */
	 ...
	 *
	 * @param index        the index of the column where the conversions should be
	 *                     executed
	...
	public FieldConversionMapping clone() {
		try {
			...
			out.fieldIndexConversionMapping = fieldIndexConversionMapping.clone(clonedSelectors,
					out.conversionSequence);
...
/**
 * Class responsible for managing field selections and any conversion sequence
 * associated with each.
 *
 * @param <T> the FieldSelector type information used to uniquely identify a
 *            field (e.g. references to field indexes would use Integer, while
 *            references to field names would use String).
 *
 * @author Univocity Software Pty Ltd -
 *         <a href="mailto:parsers@univocity.com">parsers@univocity.com</a>
...
abstract class AbstractConversionMapping<T> implements Cloneable {
	 ...
	 * <p>
	 * The selector instance that is used to store which fields should be converted
	 * is added to the {@link AbstractConversionMapping#conversionSequence} list in
	 * order to keep track of the correct conversion order.
	 * <p>
	 * This is required further conversion sequences might be added to the same
	 * fields in separate calls.
	 *
	 * @param conversions the conversion sequence to be applied to a set of fields.
	 *
	 * @return a FieldSet which provides methods to select the fields that must be
	 *         converted or null if the FieldSelector returned by #newFieldSelector
	 *         is not an instance of FieldSet (which is the case of
	 *         {@link AllIndexesSelector}).
	 */
	...
	/**
	 * Creates a FieldSelector instance of the desired type. Used in @link
	...
	/**
	 * Get all indexes in the given selector and adds the conversions defined at
	 * that index to the map of conversionsByIndex.
	 * <p>
	 * This method is called in the same sequence each selector was created (in
	 * {@link FieldConversionMapping#prepareExecution(boolean, String[])})
	 * <p>
	 * At the end of the process, the map of conversionsByIndex will have each index
	 * with its list of conversions in the order they were declared.
	 *
	 * @param writing            flag indicating whether a writing process is being
	 *                           initialized.
	 * @param selector           the selected fields for a given conversion
	 *                           sequence.
	 * @param conversionsByIndex map of all conversions registered to every field
	 *                           index, in the order they were declared
	 * @param values             The field sequence that identifies how records will
	 *                           be organized.
	 *                           <p>
	 *                           This is generally the sequence of headers in a
	 *                           record, but it might be just the first parsed row
	 *                           from a given input (as field selection by index is
	 *                           allowed).
	 */
	public void prepareExecution(boolean writing, FieldSelector selector,
			Map<Integer, List<Conversion<?, ?>>> conversionsByIndex, String[] values) {
		if (conversionsMap == null) {
			return;
		}

		// conversionsMap contains maps the conversions applied to a field selection
		// we will match the indexes where these conversions where applied and add them
		// to the corresponding list in conversionsByIndex
	...
	/**
	 * Ensures an individual field does not have the same conversion object applied
	 * to it more than once.
	 *
	 * @param selector           the selection of fields
	 * @param conversionsAtIndex the sequence of conversions applied to a given
	 *                           index
	 * @param conversionsToAdd   the sequence of conversions to add to
	 *                           conversionsAtIndex
	 */
	private static void validateDuplicates(FieldSelector selector, List<Conversion<?, ?>> conversionsAtIndex,
			Conversion<?, ?>[] conversionsToAdd) {
		for (Conversion<?, ?> toAdd : conversionsToAdd) {
			for (Conversion<?, ?> existing : conversionsAtIndex) {
				if (toAdd == existing) {
					throw new DataProcessingException("Duplicate conversion " + toAdd.getClass().getName()
							+ " being applied to " + selector.describe());
				}
			}
		}
	}
	 ...
	 *
	 * @return true if no conversions were associated with any field; false
	 *         otherwise
	 */
	...
	public AbstractConversionMapping<T> clone(Map<FieldSelector, FieldSelector> clonedSelectors,
			List<FieldSelector> clonedConversionSequence) {

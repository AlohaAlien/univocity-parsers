...
import static com.bupt.se.annotations.helpers.MethodDescriptor.*;
...
public final class ColumnMapping implements ColumnMapper {
	...
	/**
	 * Creates a nested column mapping instance for handling nested attributes. For
	 * internal use.
	 *
	 * @param prefix the current nesting path, denoted by a dot separated string of
	 *               attribute names
	 ...
	 * @return {@code true} if the given method or attribute has been mapped to a
	 *         column
	 */
	public boolean isMapped(MethodDescriptor method, String targetName) {
		return methodMapping.isMapped(method) || attributeMapping.isMapped(targetName)
				|| methodNameMapping.isMapped(targetName);
	}
	 ...
	 * @param targetName   name of a method or attribute
	 * @param method       a descriptor of getter/setter methods (can be
	 *                     {@code null})
	 *
	...
	/**
	 * Returns the first-level names of all nested members whose attributes or
	 * methods have been mapped
	 * 

 ...
 ******************************************************************************/
package com.bupt.se.common.routine;
...
/**
 * Basic implementation of commonly used routines around parsing/writing of data
...
public abstract class AbstractRoutines<P extends CommonParserSettings<?>, W extends CommonWriterSettings<?>> {
	 ...
	 *
	 * @return a parser implementation configured according to the given settings
	 *         object.
	 */
	 ...
	 *
	 * @return a writer implementation configured according to the given settings
	 *         object.
	 */
	...
	/**
	 * Returns the parser configuration (if any) used by the routines of this
	 * utility class.
	 *
	...
	/**
	 * Defines the parser configuration to be used by the routines of this utility
	 * class.
	 *
	...
	/**
	 * Returns the writer configuration (if any) used by the routines of this
	 * utility class.
	 *
	...
	/**
	 * Defines the writer configuration to be used by the routines of this utility
	 * class.
	 *
	...
	/**
	 * Allows writers of any given format to adjust its settings to take into
	 * account column headers and lengths
	 ...
	 *
	 * @param rs     the {@link java.sql.ResultSet} whose contents should be read
	 *               and written to a given output
	 * @param output the output file that will store the data in the given
	 *               {@link java.sql.ResultSet}
	 *               in the format specified by concrete implementations of this
	 *               class.
	 */
	 ...
	 * @param rs     the {@link java.sql.ResultSet} whose contents should be read
	 *               and written to a given output
	 * @param output the output stream that will store the data in the given
	 *               {@link java.sql.ResultSet}
	 *               in the format specified by concrete implementations of this
	 *               class.
	 */
	 ...
	 * @param rs     the {@link java.sql.ResultSet} whose contents should be read
	 *               and written to a given output
	 * @param output the output that will store the data in the given
	 *               {@link java.sql.ResultSet}
	 *               in the format specified by concrete implementations of this
	 *               class.
	 */
			...
			try {
				...
				for (int i = 1; i <= columns; i++) {
					...
					if (precision != 0 && scale != 0) {
						length = precision + scale + 2; // +2 to account for decimal point (non-integer numbers) and
														// minus characters (negative numbers).
					} else {
	 ...
	 *
	 * @param input  the input data to be parsed using the settings provided in
	 *               {@link #getParserSettings()}
	 * @param output the output into where the input data should be written, using
	 *               the format provided in {@link #getWriterSettings()}
	 */
	 ...
	 * @param <T>      the type of element in the given collection
	 */
	public <T> void writeAll(Iterable<T> elements, Class<T> beanType, File output, Charset encoding,
			String... headers) {
		writeAll(elements, beanType, ArgumentUtils.newWriter(output, encoding), headers);
	}
	 ...
	 * @param <T>      the type of element in the given collection
	 */
	public <T> void writeAll(Iterable<T> elements, Class<T> beanType, OutputStream output, String encoding,
			String[] headers) {
		writeAll(elements, beanType, ArgumentUtils.newWriter(output, encoding), headers);
	}
	 ...
	 * @param <T>      the type of element in the given collection
	 */
	public <T> void writeAll(Iterable<T> elements, Class<T> beanType, OutputStream output, Charset encoding,
			String... headers) {
		writeAll(elements, beanType, ArgumentUtils.newWriter(output, encoding), headers);
	}
	 ...
	 * @param expectedBeanCount expected number of rows to be parsed from the input
	 *                          which will be converted into java beans.
	 *                          Used to pre-allocate the size of the output
	 *                          {@link List}
	 * @param <T>               the type of java beans to be instantiated.
	 *
	 * @return an {@link Iterable} that allows iterating over the input and
	 *         producing instances of java beans on demand.
	 */
	...
	/**
	 * Iterates over an input stream to produce instances of annotated java beans on
	 * demand.
	 *
	 * @param beanType the type of java beans to be instantiated.
	 * @param input    the input stream to be parsed
	...
	/**
	 * Iterates over an input to produce instances of annotated java beans on
	 * demand.
	 *
	 * @param beanType the type of java beans to be instantiated.
	 * @param input    the input to be parsed
	 ...
	 * @param input the file to be parsed
	 *
	 * @return a {@link InputDimension} with information about the dimensions of the
	 *         given input.
	 */
	...
	public InputDimension getInputDimension(Reader input) {
		...
		// By disabling column reordering, we get the original row, with nulls in the
		// columns as nothing was selected.
	...
	/**
	 * Returns a flag indicating whether resources used for writing should be kept
	 * open after being
	 * used by the routines available from this object, when applicable.
	 *
	 * @return flag indicating whether to call the {@code close()} (or any other
	 *         cleanup method)
	 ...
	 * used by the routines available from this object, when applicable.
	 *
	 * @param keepResourcesOpen flag indicating whether to call the {@code close()}

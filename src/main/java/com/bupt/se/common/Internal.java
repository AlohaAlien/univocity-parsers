...
/**
 * Internal class to keep common internal functions that are used in multiple
 * places.
 *
 ...
 */
class Internal {
	public static final <C extends Context> void process(String[] row, Processor<C> processor, C context,
		...
		try {
			...
			if (!ex.isFatal() && !ex.isHandled() && ex.getColumnIndex() > -1
					&& errorHandler instanceof RetryableErrorHandler) {
	...
	private static final void throwDataProcessingException(Processor processor, Throwable t, String[] row,
			int errorContentLength) throws DataProcessingException {
		DataProcessingException ex = new DataProcessingException("Unexpected error processing input row "
				...
				AbstractException.restrictContent(errorContentLength, row), t);

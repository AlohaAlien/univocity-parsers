...
class RecordMetaDataImpl<C extends Context> implements RecordMetaData {
	...
	private MetaData getMetaData(String name) {
		...
		if (index == -1) {
			...
			throw new IllegalArgumentException(
					"Header name '" + name + "' not found. Available columns are: " + Arrays.asList(selectedHeaders()));
		}
	...
	private <T> T convert(MetaData md, String[] data, Class<T> type, T defaultValue, Annotation annotation) {
		...
		try {
			...
			DataProcessingException exception = new DataProcessingException(
					"Cannot cast value '{value}' of type " + out.getClass().toString() + " to " + type.getName());
			exception.setValue(out);
			exception.setErrorContentLength(context.errorContentLength());
			throw exception;
		}
	...
	<T> T getObjectValue(String[] data, String headerName, Class<T> type, T defaultValue, String format,
	...
	<T> T getObjectValue(String[] data, int columnIndex, Class<T> type, T defaultValue, String format,
			String... formatOptions) {
		if (format == null) {
	...
	<T> T getObjectValue(String[] data, Enum<?> column, Class<T> type, T defaultValue, String format,
			String... formatOptions) {
		if (format == null) {
	...
	static Annotation buildBooleanStringAnnotation(final String[] trueStrings, final String[] falseStrings) {
		return new com.bupt.se.annotations.BooleanString() {
			@Override
	...
	private static Annotation newFormatAnnotation(final String format, final String... formatOptions) {
		return new com.bupt.se.annotations.Format() {
			...
			@Override
			public Class<? extends Annotation> annotationType() {
				return com.bupt.se.annotations.Format.class;
			}
		};
	}

	<T> Annotation buildAnnotation(Class<T> type, final String args1, final String... args2) {
		Integer hash = (type.hashCode() * 31) + String.valueOf(args1).hashCode()

...
/**
 * A simple cache of values associated with strings. It is built to simply
 * prevent generating the same value over
 * and over again over a short period of time. Once its size limit is reached,
 * the cache will be fully cleared.
 * Do not use this as a general purpose caching solution. This meant for storing
 * values that can be cheaply produced
 * and re-generating them every now and then won't incur in any major
 * performance impact.
 *
...
public abstract class StringCache<T> {
	 ...
	 * @return {@code true} if the cache contains (or contained) a value associated
	 *         with the given key.
	 */
	...
	/**
	 * Returns the size limit of this string cache. Defaults to 16,384. For
	 * simplicity, when
	 * this limit is reached, the entire cache is cleared.
	 *
	 * @return the maximum number of entries that can be stored in this string
	 *         cache.
	 */
	...
	/**
	 * Defines the size limit of this string cache (16,384 by default). For
	 * simplicity, when
	 * this limit is reached, the entire cache is cleared.
	 *
	 * @param sizeLimit the maximum number of entries that can be stored in this
	 *                  string cache.
	 */
	 ...
	 * or if it has been evicted, a value will be populated using
	 * {@link #process(String)}
	 *
	...
	/**
	 * Returns the maximum length a {@code String} key can have to be used as a key
	 * in this cache.
	 * If the {@code String} length exceeds this limit, the value associated with it
	 * won't be cached.
	 * Defaults to 1024
	 *

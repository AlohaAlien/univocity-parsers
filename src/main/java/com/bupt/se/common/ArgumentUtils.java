...
public class ArgumentUtils {
	 ...
	 * @param fieldSelector a field selector that indicates which elements of the
	 *                      given array are selected.
	 *
	 ...
	 *                search
	 *
	 * @return the index of the given character in the array, or -1 if the character
	 *         could not be found.
	 */
	...
	public static Reader newReader(InputStream input, Charset encoding) {
		if (encoding == null) {
			...
			if (bomInput.getEncoding() != null) { // charset detected. Just set the encoding and keep using the original
													// input stream.
				encoding = bomInput.getCharset();
			}

			if (bomInput.hasBytesStored()) { // there are bytes to be processed. We should use the BomInput wrapper to
												// read the first bytes already consumed when trying to match the BOM.
				input = bomInput;
			} // else the original input can be used and the wrapper is not necessary, as a
				// BOM has been matched and the bytes discarded.
		}
	 ...
	 * as this generally causes more trouble than it solves (your exception-specific
	 * catch statement may not catch this
	 ...
	 * of `\r` and `\n` with `[cr]` and `[lf]`.
	 * If `\r` is followed by `\n` or vice versa, then `[crlf]` or `[lfcr]` will be
	 * printed.
	 *
	 ...
	 *                   if {@code true}, `\r` will be replaced by `[cr]\r` for
	 *                   example.
	 *

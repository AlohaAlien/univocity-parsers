 ...
 * {@link CharInputReader}.
 *
 * <p>
 * It provides the essential conversion of sequences of newline characters
 * defined by {@link Format#getLineSeparator()} into the normalized newline
 * character provided in {@link Format#getNormalizedNewline()}.
 * <p>
 * It also provides a default implementation for most of the methods specified
 * by the {@link CharInputReader} interface.
 * <p>
 * Extending classes must essentially read characters from a given
 * {@link java.io.Reader} and assign it to the public
 * {@link AbstractCharInputReader#buffer} when requested (in the
 * {@link AbstractCharInputReader#reloadBuffer()} method).
 *
 * @author Univocity Software Pty Ltd -
 *         <a href="mailto:parsers@univocity.com">parsers@univocity.com</a>
 * @see com.bupt.se.common.Format
 * @see com.bupt.se.common.input.DefaultCharInputReader
 * @see com.bupt.se.common.input.concurrent.ConcurrentCharInputReader
 */
...
public abstract class AbstractCharInputReader implements CharInputReader {
	...
	/**
	 * Creates a new instance that attempts to detect the newlines used in the input
	 * automatically.
	 *
	 * @param normalizedLineSeparator the normalized newline character (as defined
	 *                                in {@link Format#getNormalizedNewline()}) that
	 *                                is used to replace any lineSeparator sequence
	 *                                found in the input.
	 * @param whitespaceRangeStart    starting range of characters considered to be
	 *                                whitespace.
	 * @param closeOnStop             indicates whether to automatically close the
	 *                                input when {@link #stop()} is called
	 */
	 ...
	 * transparently.
	 *
	 * @param lineSeparator           the sequence of characters that represent a
	 *                                newline, as defined in
	...
	private void setLineSeparator(char[] lineSeparator) {
		...
		if (lineSeparator.length > 2) {
			throw new IllegalArgumentException("Invalid line separator. Up to 2 characters are expected. Got "
					+ lineSeparator.length + " characters.");
		}
	...
	/**
	 * Passes the {@link java.io.Reader} provided in the
	 * {@link AbstractCharInputReader#start(Reader)} method to the extending class
	 * so it can begin loading characters from it.
	 *
	 * @param reader the {@link java.io.Reader} provided in
	 *               {@link AbstractCharInputReader#start(Reader)}
	 */
	protected abstract void setReader(Reader reader);

	/**
	 * Informs the extending class that the buffer has been read entirely and
	 * requests for another batch of characters.
	 * Implementors must assign the new character buffer to the public
	 * {@link AbstractCharInputReader#buffer} attribute, as well as the number of
	 * characters available to the public {@link AbstractCharInputReader#length}
	 * attribute.
	 * To notify the input does not have any more characters,
	 * {@link AbstractCharInputReader#length} must receive the <b>-1</b> value
	 */
	...
	private void start(Reader reader, boolean resetTmp) {
		...
		// if the input has been properly decoded with the correct UTF* character set,
		// but has a BOM marker, we can safely discard it.
		if (length > 0 && buffer[0] == '\uFEFF') { // regardless of the UTF* encoding used, the BOM bytes always produce
													// the '\uFEFF' character when decoded.
			i++;
		}
	}
	 ...
	 *
	 * <p>
	 * If there are no more characters in the input, the reading will stop by
	 * invoking the {@link AbstractCharInputReader#stop()} method.
	 */
	private void updateBuffer() {
		...
		if (inputAnalysisProcesses != null) {
			if (length > 0 && length <= 4) {
				...
				char[] tmp = Arrays.copyOfRange(buffer, 0, length + 1); // length + 1 to assist CSV detection process:
																		// length < buffer.length indicates all data was
																		// read into the buffer.

				// sets processes temporarily to null to prevent them running if method
				// `unwrapInputStream` is called.
	...
	/**
	 * Submits a custom {@link InputAnalysisProcess} to analyze the input buffer and
	 * potentially discover configuration options such as
	...
	public final String getString(char ch, char stop, boolean trim, String nullValue, int maxLength) {
		...
		int pos = this.i - 1;
		int len = i - this.i;
		if (maxLength != -1 && len > maxLength) { // validating before trailing whitespace handling so this behaves as
													// an appender.
			return null;
		}
	...
	@Override
	public final String getQuotedString(char quote, char escape, char escapeEscape, int maxLength, char stop1,
			char stop2, boolean keepQuotes, boolean keepEscape, boolean trimLeading, boolean trimTrailing) {
		if (i == 0) {
			return null;
		}
		...
		while (true) {
			...
			} else if (lineSeparator1 == ch && normalizeLineEndings
					&& (lineSeparator2 == '\0' || i + 1 < length && lineSeparator2 == buffer[i + 1])) {
				return null;
			}
			i++;
		}

...
/**
 * An implementation {@link CharAppender} that expands the internal buffer of
...
public class ExpandingCharAppender extends DefaultCharAppender {
	...
	/**
	 * Creates an {@code ExpandingCharAppender} a the default value to return when
	 * no characters have been accumulated.
	...
	private void expand(int additionalLength, double factor) {
		if (chars.length == MAX_ARRAY_LENGTH) {
			throw new TextParsingException(null,
					"Can't expand internal appender array to over " + MAX_ARRAY_LENGTH + " characters in length.");
		}

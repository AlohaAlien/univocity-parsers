...
/**
 * Extension of the {@link DefaultCharAppender} class to include facilities for
 * writing to an output. Used by writers extending {@link AbstractWriter}.
 *
 * <p>
 * This class introduces the handling of the normalized newline character
 * defined in {@link Format#getNormalizedNewline()} and converts it to the
 * newline sequence in {@link Format#getLineSeparator()}
 * <p>
 * It also introduces methods to write to an instance of {@link java.io.Writer}
...
public class WriterCharAppender extends ExpandingCharAppender {
	 ...
	 * <li>the default value to return when no characters have been
	 * accumulated.</li>
	 ...
	 * @param emptyValue           default value to return when no characters have
	 *                             been accumulated
	 * @param whitespaceRangeStart starting range of characters considered to be
	 *                             whitespace.
	 * @param format               output format specification used for newline
	 *                             handling
	 */
	 ...
	 *
	 * <p>
	 * If the given character is equal to {@link Format#getNormalizedNewline()},
	 * then the character sequence returned by {@link Format#getLineSeparator()} is
	...
	/**
	 * Appends the given character and marks it as ignored if it is a whitespace
	 * ({@code ch <= ' '}) or a padding character
	 *
	 * <p>
	 * If the given character is equal to {@link Format#getNormalizedNewline()},
	 * then the character sequence returned by {@link Format#getLineSeparator()} is
	...
	/**
	 * Writes the accumulated value to the {@link java.io.Writer}, discarding any
	 * trailing whitespace characters identified when using
	 * {@link WriterCharAppender#appendIgnoringWhitespace(char)},
	 * {@link WriterCharAppender#appendIgnoringPadding(char, char)} or
	 * {@link WriterCharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 * <p>
	 * The internal accumulated value is discarded after invoking this method (as in
	 * {@link DefaultCharAppender#reset()})
	 * <p>
	 * If the accumulated value is empty (i.e. no characters were appended, or all
	 * appended characters where ignored as whitespace or padding), then the written
	 * value will be the {@link DefaultCharAppender#emptyValue} attribute defined in
	...
	/**
	 * Configures the appender to allow line ending sequences to be appended as-is,
	 * without replacing them by the
	 * normalized line separator character.
	 *
	 * @param enableDenormalizedLineEndings flag indicating whether denormalized
	 *                                      line endings are allowed. The writer
	 *                                      won't convert line separators
	 *                                      automatically.
	 */

 ...
 *
 * <p>
 * This is essentially an immutable circular queue. Elements are not added nor
 * removed. Pointers to the head and tail of the queue identify what is the next
 * available entry.
 * <p>
 * Use {@link FixedInstancePool#allocate()} to get an available {@link Entry}
 * from the pool. If all objects are allocated then the thread will block until
 * an element is released.
 * <p>
 * {@link FixedInstancePool#release(Entry)} releases an allocated {@link Entry}
 * for reuse.
 *
...
abstract class FixedInstancePool<T> {
	...
	/**
	 * Creates a new instance pool with the given size. Upon instantiation, the
	 * {@link FixedInstancePool#newInstance()} method will be called to fill in the
	 * instance pool, and the pool
	...
	/**
	 * Creates a new instance of the given type of objects stored as entries of this
	 * instance pool
	 * This method is called in the constructor of this class for initialization of
	 * the instance array and must always return a new instance.
	 *
	...
	/**
	 * Retrieves the next available entry in this instance pool. Blocks until an
	 * entry becomes available (through {@link FixedInstancePool#release(Entry)}).
	 *
	...
	public synchronized Entry<T> allocate() {
		...
		// instanceIndexes[head] = -1; //enable to print the queue's contents for
		// debugging purposes
		head++;
	 ...
	 * {@link FixedInstancePool#allocate()})
	 *

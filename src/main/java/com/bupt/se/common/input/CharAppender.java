...
/**
 *
 * The general interface for classes responsible for appending characters
 * efficiently while handling whitespaces and padding characters.
 *
 * <p>
 * Calls to {@link CharAppender#appendIgnoringWhitespace(char)},
 * {@link CharAppender#appendIgnoringPadding(char, char)} and
 * {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)} should
 * accumulate the
 * given character and only discard whitespaces/padding if no non-whitespace is
 * appended:
 *
 * <p>
 * For example:
 ...
 *
 * <p>
 * <b>Implementation note:</b> White spaces should be identified as any
 * character {@code <= ' '}
 *
...
public interface CharAppender extends CharSequence {
	...
	/**
	 * Appends the given character and marks it as ignored if it is a padding
	 * character (the definition of a padding character is implementation
	 * dependent.)
	 * 
	 * @param ch      character to append
	...
	/**
	 * Appends the given character and marks it as ignored if it is a whitespace
	 * ({@code ch <= ' '}) or a padding character (the definition of a padding
	 * character is implementation dependent.)
	 * 
	 * @param ch      character to append
	 * @param padding the padding character to ignore
	 */
	 ...
	 * @return the position of the given character in the appended content,
	 *         {@code -1} if not found
	 */
	 ...
	 * @param from         the starting index from where the search will begin.
	 * @return the position of the given character sequence in the appended content,
	 *         {@code -1} if not found
	 */
	 ...
	 * @param from  the starting index from where the search will begin.
	 * @return the position any one of the given characters in the appended content,
	 ...
	 * @param length the number of characters to accumulate from the given start
	 *               position
	 * @return a {@code String} with the section of characters accumulated by this
	 *         appender.
	 */
	...
	/**
	 * Returns the current accumulated value length (the sum of all appended
	 * characters - whitespaceCount).
	 * 
	 * @return the current accumulated value length (the sum of all appended
	 *         characters - whitespaceCount).
	 */
	int length();

	/**
	 * Returns the current number of whitespaces accumulated after the last
	 * non-whitespace character.
	 * <p>
	 * This is the number of whitespaces accumulated using
	 * {@link CharAppender#appendIgnoringWhitespace(char)},
	 * {@link CharAppender#appendIgnoringPadding(char, char)} or
	 * {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 * 
	 * @return the number of whitespaces accumulated using
	 *         {@link CharAppender#appendIgnoringWhitespace(char)},
	 *         {@link CharAppender#appendIgnoringPadding(char, char)} or
	 *         {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 */
	...
	/**
	 * Resets the number of whitespaces accumulated after the last non-whitespace
	 * character.
	 * <p>
	 * This is the number of whitespaces accumulated using
	 * {@link CharAppender#appendIgnoringWhitespace(char)},
	 ...
	 * {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 * <p>
	 * A subsequent call to {@link CharAppender#whitespaceCount()} should return 0.
	 */
	void resetWhitespaceCount();

	/**
	 * Returns the accumulated value as a String, discarding any trailing whitespace
	 * characters identified when using
	 * {@link CharAppender#appendIgnoringWhitespace(char)},
	 * {@link CharAppender#appendIgnoringPadding(char, char)} or
	 * {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 * <p>
	 * The internal accumulated value is discarded after invoking this method (as in
	 * {@link CharAppender#reset()})
	 * 
	 * @return a String containing the accumulated characters without the trailing
	 *         whitespaces.
	 */
	String getAndReset();
	...
	/**
	 * Returns the accumulated characters, discarding any trailing whitespace
	 * characters identified when using
	 * {@link CharAppender#appendIgnoringWhitespace(char)},
	 * {@link CharAppender#appendIgnoringPadding(char, char)} or
	 * {@link CharAppender#appendIgnoringWhitespaceAndPadding(char, char)}
	 ...
	 * {@link CharAppender#reset()})
	 * 
	 * @return a character array containing the accumulated characters without the
	 ...
	 * 
	 * @param ch1 the first character to prepend in front of the current accumulated
	 *            value.
	 * @param ch2 the second character to prepend in front of the current
	 *            accumulated value.
	 */
	 ...
	 * 
	 * @param chars the character sequence to prepend in front of the current
	 *              accumulated value.
	 */
	void prepend(char[] chars);

	/**
	 * Updates the internal whitespace count of this appender to trim trailing
	 * whitespaces.
	 */
	 ...
	 * effectively marking these as whitespace. Invoking
	 * {@link #resetWhitespaceCount()}
	 ...
	 * Will reset the internal whitespace count if any. Invoke
	 * {@link #updateWhitespace()}
	 ...
	 * 
	 * @return {@code} true calling {@link #getAndReset()} would return
	 *         {@code null}, otherwise {@code false}.
	 */
	boolean isEmpty();

	/**
	 * Returns the last index of a given character in the current appended
	 * (characters that have been marked as whitespace will be ignored)
	 * 
	 * @param ch the character to look for
	 * @return the last position of the given character in the appended content, or
	 *         {@code -1} if not found.
	 */

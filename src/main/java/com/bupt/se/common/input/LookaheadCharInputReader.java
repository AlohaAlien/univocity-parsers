...
/**
 * A special implementation of {@link CharInputReader} that wraps another
 * {@link CharInputReader} and
 * collects a sequence of characters from the wrapped input, in order to analyze
...
public class LookaheadCharInputReader implements CharInputReader {
	...
	/**
	 * Creates a lookahead input reader by wrapping a given {@link CharInputReader}
	 * implementation
	 *
	 * @param reader               the input reader whose characters will read and
	 *                             stored in a limited internal buffer,
	 *                             in order to allow a parser to query what the
	 *                             characters are available ahead of the current
	 *                             input position.
	 * @param newLine              the normalized character that represents a line
	 *                             ending. Used internally as a stop character.
	 ...
	 *
	 * @param current  the last character used by the parser, which should match the
	 *                 first character in the lookahead buffer
	 * @param sequence the expected sequence of characters after the current
	 *                 character, that are expected appear in the current lookahead
	 *                 buffer
	 * @param wildcard character used in the sequence as a wildcard (e.g. * or ?),
	 *                 meaning any character is acceptable in its place.
	 *
	 * @return {@code true} if the current character and the sequence characters
	 *         that follows are present in the lookahead, otherwise {@code false}
	 */
	 ...
	 * Matches a sequence of characters against the current lookahead buffer.
	 *
	 * @param sequence the expected sequence of characters that are expected appear
	 *                 in the current lookahead buffer
	 * @param wildcard character used in the sequence as a wildcard (e.g. * or ?),
	 *                 meaning any character is acceptable in its place.
	 *
	 * @return {@code true} if the given sequence of characters is present in the
	 *         lookahead, otherwise {@code false}
	 */
	 ...
	 *
	 * @return the current lookahead value, or an empty {@code String} if the
	 ...
	 *
	 * @return a {@code String} formed by the given character followed by the
	 *         lookahead value (if any).
	 */
	...
	/**
	 * Fills the lookahead buffer with a given number of characters that will be
	 * extracted from the wrapped {@link CharInputReader}
	 *
	 * @param numberOfCharacters the number of characters to read from the wrapped
	 *                           {@link CharInputReader}, given in the constructor
	 *                           of this class.
	 */
	...
	@Override
	public String getQuotedString(char quote, char escape, char escapeEscape, int maxLength, char stop1, char stop2,

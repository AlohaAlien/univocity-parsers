...
/**
 * A concurrent character loader for loading a pool of {@link CharBucket}
 * instances using a {@link java.io.Reader} in a separate thread
 *
...
class ConcurrentCharLoader implements Runnable {
	...
	/**
	 * Creates a {@link FixedInstancePool} with a given amount of {@link CharBucket}
	 * instances and starts a thread to fill each one.
	 *
	 * @param reader         The source of characters to extract and fill
	 *                       {@link CharBucket} instances
	 * @param bucketSize     The size of each individual {@link CharBucket}
	 * @param bucketQuantity The number of {@link CharBucket} instances used to
	 *                       extract characters from the given reader.
	 * @param closeOnStop    Indicates whether to automatically close the input when
	 *                       {@link #stopReading()} is called
	 */
	...
	/**
	 * The {@link CharBucket} loading process that executes in parallel until the
	 * input is completely read.
	 * Once the end of the input is reached, the {@link java.io.Reader} instance
	 * provided in the constructor is closed.
	 */
	@Override
	public void run() {
		try {
	...
	private void setError(Exception e) {
		if (active) {
			error = e;
		} // else if not active then input was closed externally - we can ignore the
			// exception.
	}

	/**
	 * Returns the next available bucket. Blocks until a bucket is made available or
	...
	/**
	 * Stops the {@link CharBucket} loading process and closes the reader provided

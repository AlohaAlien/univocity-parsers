...
/**
 * A wrapper for an {@link InputStream} that attempts to detect a Byte Order
 * Mark (BOM) in the input
 * and derive the character encoding that should be used to decode the incoming
 * content.
 *
...
public final class BomInput extends InputStream {
	...
	/**
	 * Wraps an {@link InputStream} and reads the first bytes found on it to attempt
	 * to read a BOM.
	 *
	...
	public BomInput(InputStream input) {
		...
		try { // This looks shitty on purpose (all in the name of speed).
			...
			// store the exception for later. We want the wrapper to behave exactly like the
			// original input stream and
	...
	public final int read() throws IOException {
		if (bytesRead > 0 && bytesRead > consumed) {
			...
			// Ensures that if the original input stream returned a byte, it will be
			// consumed.
			// In case of exceptions, bytes produced prior to the exception will still be
			// returned.
	...
	/**
	 * Returns a flag indicating whether or not all bytes read from the wrapped
	 * input stream have been consumed. This
	 * allows client code to determine if the original input stream can be used
	 * directly and safely, or if this
	 * {@code BomInput} wrapper class should be used instead.
	 *
	 * If there are stored bytes that need to be consumed before the wrapped input
	 * stream is consumed again,
	 * this method will return {@code true}.
	 *
	 * @return {@code false} if there are no bytes stored and the original input
	 *         stream can be used directly. If this wrapper
	 *         needs to be used to return stored bytes before, then {@code true}
	 *         will be returned.
	 */
	...
	/**
	 * Returns the detected {@link Charset} determined by the Byte Order Mark (BOM)
	 * available in the
	 * input provided in the constructor of this class.
	 *
	 * If no BOM was detected, this method will return {@code null}.
	 *
	 * @return the detected {@link Charset} or {@code null} if a BOM could not be
	 *         matched.
	 */
	...
	/**
	 * Returns the detected encoding name determined by the Byte Order Mark (BOM)
	 * available in the
	 * input provided in the constructor of this class.
	 *
	 * If no BOM was detected, this method will return {@code null}.
	 *
	 * @return the detected encoding name or {@code null} if a BOM could not be
	 *         matched.
	 */
	 ...
	 * {@link InputStream} into a {@link Reader}.
	 * This is required for performance reasons as overriding
	 * {@link InputStream#read()} incurs a heavy performance
	 * penalty when the implementation is native (as in

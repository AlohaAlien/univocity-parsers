...
/**
 *
 * A {@link Processor} implementation for associating rows extracted from any
 * implementation of {@link AbstractParser} into {@link MasterDetailRecord}
 * instances.
 *
 * <p>
 * For each row processed, a call to
 * {@link AbstractMasterDetailProcessor#isMasterRecord(String[], Context)} will
 * be made to identify whether or not it is a master row.
 * <p>
 * The detail rows are automatically associated with the master record in an
 * instance of {@link MasterDetailRecord}.
 * <p>
 * When the master record is fully processed (i.e. {@link MasterDetailRecord}
 * contains a master row and all associated detail rows),
 * it is sent to the user for processing in
 * {@link AbstractMasterDetailProcessor#masterDetailRecordProcessed(MasterDetailRecord, Context)}.
 *
 * <p>
 * <b>Note</b> this class extends {@link AbstractObjectProcessor} and value
 * conversions provided by {@link Conversion} instances are fully supported.
 *
...
public abstract class AbstractMasterDetailProcessor<T extends Context> extends AbstractObjectProcessor<T> {
	 ...
	 *
	 * @param rowPlacement    indication whether the master records are placed in
	 *                        relation its detail records in the input.
	 *
	 *                        <hr>
	 *                        <blockquote>
	 * 
	 ...
	 *                        </blockquote>
	 *                        <hr>
	 * @param detailProcessor the {@link ObjectRowListProcessor} that processes
	 *                        detail rows.
	 */
	...
	/**
	 * Creates a MasterDetailProcessor assuming master records are positioned above
	 * its detail records in the input.
	 *
	 * @param detailProcessor the {@link AbstractObjectListProcessor} that processes
	 *                        detail rows.
	 */
	 ...
	 *
	 * <p>
	 * This method will then try to identify whether the given record is a master
	 * record.
	 * <p>
	 * If it is, any conversions applied to the fields of the master record will be
	 * executed;
	 * <p>
	 * Otherwise, the parsed row will be delegated to the
	 * {@link AbstractMasterDetailProcessor#detailProcessor} given in the
	 * constructor, and a detail record will be associated with the current
	 * {@link MasterDetailRecord}
	 *
	 *
	...
	/**
	 * Invoked by the parser after all values of a valid record have been processed
	 * and any conversions have been executed.
	 *
	 * @param row     the data extracted by the parser for an individual record.
	 * @param context A contextual object with information and controls over the
	 *                current state of the parsing process
	...
	/**
	 * Associates individual rows to a {@link MasterDetailRecord} and invokes
	 * {@link AbstractMasterDetailProcessor#masterDetailRecordProcessed(MasterDetailRecord, T)}
	 * when it is fully populated.
	 * 
	 * @param row     a record extracted from the parser that had all (if any)
	 *                conversions executed and is ready to be sent to the user.
	 * @param context A contextual object with information and controls over the
	 *                current state of the parsing process
	 */
	...
	/**
	 * Invoked by the processor after a master row and all associated detail rows
	 * have been processed.
	 *

...
/**
 * A concrete implementation of {@link Processor} that allows switching among
...
public abstract class AbstractInputValueSwitch<T extends Context> extends AbstractProcessorSwitch<T> {
	...
	/**
	 * Creates a switch that will analyze the first column of rows found in the
	...
	/**
	 * Creates a switch that will analyze a column of rows parsed from the input to
	 * determine which
	 * {@link Processor} to use.
	 *
	 * @param columnIndex the column index whose value will be used to determine
	 *                    which {@link Processor} to use for each parsed row.
	 */
	...
	/**
	 * Creates a switch that will analyze a column in rows parsed from the input to
	 * determine which
	 * {@link Processor} to use.
	 *
	 * @param columnName name of the column whose values will be used to determine
	 *                   which {@link Processor} to use for each parsed row.
	 */
	 ...
	 * {@link #addSwitchForValue(String, Processor, String...)}
	 * with the column given in the constructor of this class.
	 *
	 * @param caseSensitive a flag indicating whether the switch should compare
	 *                      values not considering the character case.
	 */
	...
	/**
	 * Configures the switch to use a custom {@link Comparator} to compare values in
	 * the column to analyze which is given in the constructor of this class.
	 *
	 * @param comparator the comparator to use for matching values in the input
	 *                   column with the values provided in
	 *                   {@link #addSwitchForValue(String, Processor, String...)}
	 */
	...
	/**
	 * Defines a default {@link Processor} implementation to use when no matching
	 * value is found in the input row.
	 *
	 * @param processor    the default processor implementation
	 * @param headersToUse the (optional) sequence of headers to assign to the
	 *                     {@link ParsingContext} of the given processor
	 */
	 ...
	 *
	 * @param processor    the default processor implementation
	 * @param indexesToUse the (optional) sequence of column indexes to assign to
	 *                     the {@link ParsingContext} of the given processor
	 */
	 ...
	 * {@link Processor} implementation to use when no matching value is found in
	 * the input row.
	 * 
	 * @return {@code true} if a {@link Processor} implementation has been provided
	 *         to process input rows that doesn't have any matching value.
	 */
	...
	/**
	 * Associates a {@link Processor} implementation with an expected value to be
	 * matched in the column provided in the constructor of this class.
	 *
	 * @param value     the value to match against the column of the current input
	 *                  row and trigger the usage of the given processor
	 *                  implementation.
	 * @param processor the processor implementation when the given value matches
	 *                  with the contents in the column provided in the constructor
	 *                  of this class.
	 */
	 ...
	 * matched in the column provided in the constructor of this class.
	 *
	 * @param value        the value to match against the column of the current
	 *                     input row and trigger the usage of the given processor
	 *                     implementation.
	 * @param processor    the processor implementation when the given value matches
	 *                     with the contents in the column provided in the
	...
	/**
	 * Associates a {@link Processor} implementation with a custom matching
	 * algorithm to be executed in the column provided in the constructor of this
	 * class.
	 *
	 * @param matcher   a user defined matching implementation to execute against
	 *                  the values in the column of the current input row and
	 *                  trigger the usage of the given processor implementation.
	 * @param processor the processor implementation when the given value matches
	 *                  with the contents in the column provided in the constructor
	 *                  of this class.
	 */
	...
	protected final Processor<T> switchRowProcessor(String[] row, T context) {
		if (columnIndex == -1) {
			...
			if (headers == null) {
				throw new DataProcessingException("Unable to determine position of column named '" + columnName
						+ "' as no headers have been defined nor extracted from the input");
			}
			columnIndex = ArgumentUtils.indexOf(headers, columnName);
			if (columnIndex == -1) {
				throw new DataProcessingException("Unable to determine position of column named '" + columnName
						+ "' as it does not exist in the headers. Available headers are " + Arrays.toString(headers));
			}
		}
		...
		if (columnIndex < row.length) {
			...
			for (int i = 0; i < switches.length; i++) {
				...
				if ((s.matcher != null && s.matcher.matches(valueToMatch))
						|| comparator.compare(valueToMatch, s.value) == 0) {
					headers = s.headers;
					indexes = s.indexes;
					return s.processor;
				}
		...
		throw new DataProcessingException(
				"Unable to process input row. No switches activated and no default switch defined.", columnIndex, row,
				null);
	}

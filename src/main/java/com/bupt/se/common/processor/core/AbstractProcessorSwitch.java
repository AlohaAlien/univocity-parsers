...
/**
 * A special {@link Processor} implementation that combines and allows switching
 * among different
 * Processors. Each Processor will have its own {@link Context}. Concrete
 * implementations of this class
 * are expected to implement the {@link #switchRowProcessor(String[], Context)}
 * method and analyze the input row
 * to determine whether or not the current {@link Processor} implementation must
 * be changed to handle a special
 * circumstance (determined by the concrete implementation) such as a different
 * row format.
 *
 * When the processor is switched, the
 * {@link #processorSwitched(Processor, Processor)} will be called, and
...
public abstract class AbstractProcessorSwitch<T extends Context> implements Processor<T>, ColumnOrderDependent {
	...
	/**
	 * Analyzes the input to determine whether or not the row processor
	 * implementation must be changed
	 *
	 * @param row     a row parsed from the input
	 * @param context the current parsing context (not associated with the current
	 *                row processor used by this class)
	 *
	 * @return the row processor implementation to use. If it is not the same as the
	 *         one used by the previous row,
	 *         the returned row processor will be used, and the
	 *         {@link #processorSwitched(Processor, Processor)} method
	 *         will be called.
	 */
	protected abstract Processor<T> switchRowProcessor(String[] row, T context);

	/**
	 * Returns the headers in use by the current row processor implementation, which
	 * can vary among row processors.
	 * If {@code null}, the headers parsed by the input, or defined in
	 * {@link CommonParserSettings#getHeaders()} will be returned.
	 *
	...
	/**
	 * Returns the indexes in use by the current row processor implementation, which
	 * can vary among row processors.
	...
	/**
	 * Notifies a change of {@link Processor} implementation. Users are expected to
	 * override this method to receive the notification.
	 *
	...
	/**
	 * Notifies a change of {@link RowProcessor} implementation. Users are expected
	...
	/**
	 * Wraps a given parser context object that returns headers and extracted field
	 * indexes

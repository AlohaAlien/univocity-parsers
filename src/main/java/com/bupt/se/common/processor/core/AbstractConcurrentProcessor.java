...
/**
 * A {@link Processor} implementation to perform row processing tasks in
 * parallel. The {@code ConcurrentRowProcessor}
 * wraps another {@link Processor}, and collects rows read from the input.
 * The actual row processing is performed in by wrapped {@link Processor} in a
 * separate thread.
 *
 * <i>Note: </i> by default the {@link Context} object passed on to the wrapped
 * {@link Processor} will <b>not</b> reflect the
 * state of the parser at the time the row as generated, but the current state
 * of the parser instead. You can enable the
 * {@link #contextCopyingEnabled} flag to generate copies of the {@link Context}
 * at the time each row was generated.
 *
...
public abstract class AbstractConcurrentProcessor<T extends Context> implements Processor<T> {
	...
	/**
	 * Creates a non-blocking {@code AbstractConcurrentProcessor}, to perform
	 * processing of rows parsed from the input in a separate thread.
	 *
	 * @param processor a regular {@link Processor} implementation which will be
	...
	/**
	 * Creates a blocking {@code ConcurrentProcessor}, to perform processing of rows
	 * parsed from the input in a separate thread.
	 *
	 * @param processor a regular {@link Processor} implementation which will be
	 *                  executed in a separate thread.
	 * @param limit     the limit of rows to be kept in memory before blocking the
	...
	/**
	 * Indicates whether this processor should persist the {@link Context} object
	 * that is sent to the wrapped {@link Processor}
	 * given in the constructor of this class, so all methods of {@link Context}
	 * reflect the parser state at the time
	 * each row was parsed.
	 *
	 * Defaults to {@code false}
	 *
	 * @return a flag indicating whether the parsing context must be persisted along
	 *         with the parsed row
	 *         so its methods reflect the state of the parser at the time the record
	 *         was produced.
	 */
	...
	/**
	 * Configures this processor to persist the {@link Context} object that is sent
	 * to the wrapped {@link Processor}
	 * given in the constructor of this class, so all methods of {@link Context}
	 * reflect the parser state at the time
	 * each row was parsed.
	 *
	 * Defaults to {@code false}
	 *
	 * @param contextCopyingEnabled a flag indicating whether the parsing context
	 *                              must be persisted along with the parsed row
	 *                              so its methods reflect the state of the parser
	 *                              at the time the record was produced.
	 */

 ...
 * {@link Processor}. Used to centralize common code used by implementations
 * of {@link ColumnReader}, namely:
 * {@link AbstractColumnProcessor}, {@link AbstractObjectColumnProcessor},
 * {@link AbstractBatchedColumnProcessor} and
 * {@link AbstractBatchedObjectColumnProcessor}.
 *
 * @see ColumnReader
...
class ColumnSplitter<T> {
	...
	/**
	 * Creates a splitter allocating a space for a give number of expected rows to
	 * be read
	 * 
	 ...
	 * Removes any column values previously processed, as well as information about
	 * headers in the input. Resets row count to 0.
	 */
	void reset() {
		this.columnValues = null;
		this.headers = null;
	 ...
	 * 
	 * @return a list of lists. The stored lists correspond to the position of the
	 *         column processed from the input; Each list
	 *         contains the corresponding values parsed for a column, across
	 *         multiple rows.
	 */
	...
	/**
	 * Returns the headers of the input. This can be either the headers defined in
	 * {@link CommonSettings#getHeaders()}
	 * or the headers parsed in the input when {@link CommonSettings#getHeaders()}
	 * equals to {@code true}
	 * 
	...
	/**
	 * Initializes the list of column values, the headers of each column and which
	 * columns to read if fields
	 * have been selected using {@link CommonSettings#selectFields(String...)} or
	 * {@link CommonSettings#selectIndexes(Integer...)}
	 * 
	 * @param context the current active parsing context, which will be used to
	 *                obtain information about headers and selected fields.
	 */
	 ...
	 * @return the name of the column at the given index, or null if there's no
	 *         header defined for the given column index.
	 */
	...
	/**
	 * Splits the row and add stores the value of each column in its corresponding
	 * list in {@link #columnValues}
	 * 
	...
	void addValuesToColumns(T[] row, Context context) {
		...
		// if we have more columns than what was parsed in the current row, we need to
		// add nulls to the remaining columns.
	 ...
	 * @throws IllegalArgumentException if a column does not have a name associated
	 *                                  to it. In this case, use
	 *                                  {@link #putColumnValuesInMapOfIndexes(Map)}
	 *                                  instead.
	 */
	void putColumnValuesInMapOfNames(Map<String, List<T>> map) {
		...
		for (int i = 0; i < columnValues.size(); i++) {
			...
			if (header == null) {
				throw new DataProcessingException("Parsed input does not have header for column at index '" + i
						+ "'. Parsed header names: " + Arrays.toString(getHeaders()), i);
			}
	...
	<V> List<V> getColumnValues(int columnIndex, Class<V> columnType) {
		...
		if (columnIndex >= columnValues.size()) {
			throw new IllegalArgumentException(
					"Column index must be less than " + columnValues.size() + ". Got " + columnIndex);
		}
	...
	<V> List<V> getColumnValues(String columnName, Class<V> columnType) {
		...
		if (index == -1) {
			throw new IllegalArgumentException("No column named '" + columnName
					+ "' has been found. Available column headers: " + Arrays.toString(headers));
		}
	...
	/**
	 * Returns a map of column names and their respective list of values parsed from
	 * the input.
	 * 
	...
	/**
	 * Returns a map of column indexes and their respective list of values parsed
	 * from the input.
	 * 

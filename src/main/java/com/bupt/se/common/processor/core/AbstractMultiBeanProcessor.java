 ...
 * implementation of {@link AbstractParser} into java objects.
 *
 * <p>
 ...
 *
 * <p>
 * For each row processed, one or more java bean instances of any given class
 * will be created with their fields populated.
 * <p>
 * Each individual instance will then be sent to the
 * {@link AbstractMultiBeanProcessor#beanProcessed(Class, Object, Context)}
...
public abstract class AbstractMultiBeanProcessor<C extends Context> implements Processor<C>, ConversionProcessor {
	...
	/**
	 * Returns the {@link com.bupt.se.common.processor.BeanProcessor} responsible
	 * for processing a given class
	 *
	 * @param type the type of java bean being processed
	 * @param <T>  the type of java bean being processed
	 *
	 * @return the {@link com.bupt.se.common.processor.BeanProcessor} that handles
	 *         java beans of the given class.
	 */
	public <T> AbstractBeanProcessor<T, C> getProcessorOfType(Class<T> type) {
		...
		if (processor == null) {
			throw new IllegalArgumentException("No processor of type '" + type.getName()
					+ "' is available. Supported types are: " + processorMap.keySet());
		}
		return processor;
	}

	/**
	 * Invoked by the processor after all values of a valid record have been
	 * processed and converted into a java object.
	 *
	 * @param beanType     the type of the object created by the parser using the
	 *                     information collected for an individual record.
	 * @param beanInstance java object created with the information extracted by the
	 *                     parser for an individual record.
	 * @param context      A contextual object with information and controls over

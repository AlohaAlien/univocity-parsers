 ...
 */
package com.bupt.se.common.processor.core;

import static com.bupt.se.annotations.helpers.AnnotationHelper.*;
...
/**
 * The base class for {@link Processor} and {@link RowWriterProcessor}
 * implementations that support java beans annotated with the annotations
 * provided in
...
public class BeanConversionProcessor<T> extends DefaultConversionProcessor {
	...
	/**
	 * Initializes the BeanConversionProcessor with the annotated bean class. If any
	 * method of the given class has annotations,
	 * only the setter methods will be used (getters will be ignored), making this
	 * processor useful mostly for parsing into
	 * instances of the given class.
	 *
	 * @param beanType the class annotated with one or more of the annotations
	 *                 provided in {@link com.bupt.se.annotations}.
	 *
	 * @deprecated Use the {@link #BeanConversionProcessor(Class, MethodFilter)}
	 *             constructor instead.
	 */
	@Deprecated
	 ...
	 * @param beanType     the class annotated with one or more of the annotations
	 *                     provided in {@link com.bupt.se.annotations}.
	 * @param methodFilter filter to apply over annotated methods when the processor
	 *                     is reading data from beans (to write values to an output)
	 *                     or writing values into beans (when parsing). It is used
	 *                     to choose either a "get" or a "set"
	...
	/**
	 * Returns a flag indicating whether all headers declared in the annotated class
	 * must be present in the input.
	 * If enabled, an exception will be thrown in case the input data does not
	 * contain all headers required.
	 *
	 ...
	 *
	 * This allows users to use instances of classes that are not annotated with
	 * {@link Parsed} nor
	...
	/**
	 * Defines whether all headers declared in the annotated class must be present
	 * in the input.
	 * If enabled, an exception will be thrown in case the input data does not
	 * contain all headers required.
	 *
	 * @param strictHeaderValidationEnabled flag indicating whether strict
	 *                                      validation of headers is enabled.
	 */
	...
	void processField(AnnotatedElement element, String targetName, PropertyWrapper propertyDescriptor,
	...
	private void processNestedField(Class nestedType, AnnotatedElement element, String targetName,
			PropertyWrapper propertyDescriptor, NormalizedString[] headers, Nested nested) {
	...
	/**
	 * Creates a copy of the manually defined conversions to be applied over any
	 * columns.
	 *
	 ...
	 *
	 * @return {@code true} if the given field should be processed, otherwise
	 *         {@code false}.
	 */
	...
	void validateMappings() {
		...
		if (duplicateIndexes.size() > 0 || duplicateNames.size() > 0) {
			StringBuilder msg = new StringBuilder(
					"Conflicting field mappings defined in annotated class: " + this.getBeanClass().getName());
			for (FieldMapping mapping : duplicateIndexes) {
				msg.append("\n\tIndex: '").append(mapping.getIndex()).append("' of  ")
						.append(describeField(mapping.getTarget()));
			}
			for (FieldMapping mapping : duplicateNames) {
				msg.append("\n\tName: '").append(mapping.getFieldName()).append("' of ")
						.append(describeField(mapping.getTarget()));
			}
	...
	/**
	 * Goes through each field and method annotated with {@link Parsed} and extracts
	 * the sequence of {@link Conversion} elements associated with each one.
	 *
	 * @param target  the field and method annotated with {@link Parsed} that must
	 *                be associated with one or more {@link Conversion} objects
	 * @param mapping a helper class to store information how the field or method is
	 *                mapped to a parsed record.
	 */
	...
	private void setupConversions(AnnotatedElement target, FieldMapping mapping) {
		...
		if (!annotations.isEmpty()) {
			...
			for (Annotation annotation : annotations) {
				try {
					Conversion conversion = AnnotationHelper.getConversion(targetType, target, annotation, nullRead,
							nullWrite);
					if (conversion != null) {
					...
					throw new DataProcessingException("Error processing annotation '" + path + ". " + ex.getMessage(),
							ex);
				}
			}
		...
		boolean applyDefaultConversion = parsed == null || AnnotationRegistry.getValue(target, parsed,
				"applyDefaultConversion", parsed.applyDefaultConversion());
	...
	private boolean applyDefaultConversion(Conversion lastConversionApplied, Conversion defaultConversion) {
		...
		if (lastConversionApplied.getClass() == defaultConversion.getClass()) {
			// no need to add the default conversion as it was manually specified by the
			// user with his settings
			return false;
		}
		...
		return !(execute.getReturnType() == defaultExecute.getReturnType()
				&& revert.getReturnType() == defaultRevert.getReturnType());
	...
	private Method getConversionMethod(Conversion conversion, String methodName) {
		...
		for (Method method : conversion.getClass().getMethods()) {
			if (method.getName().equals(methodName) && !method.isSynthetic() && !method.isBridge()
					&& ((method.getModifiers() & Modifier.PUBLIC) == 1) && method.getParameterTypes().length == 1
					&& method.getReturnType() != void.class) {
				if (targetMethod != null) {
					throw new DataProcessingException(
							"Unable to convert values for class '" + beanClass + "'. Multiple '" + methodName
									+ "' methods defined in conversion " + conversion.getClass() + '.');
				}
				targetMethod = method;
			}
		}
		...
		// should never happen
		throw new DataProcessingException("Unable to convert values for class '" + beanClass + "'. Cannot find method '"
				+ methodName + "' in conversion " + conversion.getClass() + '.');
	}

	/**
	 * Associates a conversion to a field of the java bean class.
	 *
	 * @param conversion The conversion object that must be executed against the
	 *                   given field
	 * @param mapping    the helper object that contains information about how a
	 *                   field is mapped.
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * Goes through a list of objects and associates each value to a particular
	 * field of a java bean instance
	 *
	 * @param instance the java bean instance that is going to have its properties
	 *                 set
	...
	void mapValuesToFields(T instance, Object[] row, Context context) {
		if (row.length > lastFieldIndexMapped) {
			...
			mapFieldIndexes(context, row, NormalizedString.toIdentifierGroupArray(context.headers()),
					context.extractedFieldIndexes(), context.columnsReordered());
		}
	 ...
	 * @param headers          The names of all fields of the record (including any
	 *                         header that is not mapped to the java bean). May be
	 *                         null if no headers have
	 *                         been defined in {@link CommonSettings#getHeaders()}
	 * @param indexes          The indexes of the headers or row that are actually
	 *                         being used. May be null if no fields have been
	 *                         selected using
	 *                         {@link CommonSettings#selectFields(String...)} or
	 *                         {@link CommonSettings#selectIndexes(Integer...)}
	 * @param columnsReordered Indicates the indexes provided were reordered and do
	 *                         not match the original sequence of headers.
	 */

	private void mapFieldIndexes(Context context, Object[] row, NormalizedString[] headers, int[] indexes,
		...
		if (context != null && !fieldsNotFound.isEmpty()) { // Trigger this validation only when reading, not writing.
			if (headers.length == 0) {
				throw new DataProcessingException("Could not find fields " + fieldsNotFound.toString()
						+ " in input. Please enable header extraction in the parser settings in order to match field names.");
			}
			if (strictHeaderValidationEnabled) {
				DataProcessingException exception = new DataProcessingException(
						"Could not find fields " + fieldsNotFound.toString() + "' in input. Names found: {headers}");
		...
		if (indexes != null) {
			...
			// reorders the fields so they are positioned in the same order as in the
			// incoming row[]
			if (columnsReordered) {
	...
	/**
	 * Converts a record with values extracted from the parser into a java bean
	 * instance.
	 *
	 ...
	 * @param row              object array that will receive the values extracted
	 *                         from java bean
	 * @param headers          The names of all fields of the record (including any
	 *                         header that is not mapped to the java bean). May be
	 *                         null if no headers have
	 ...
	 *                         not match the original sequence of headers.
	 */
	private void mapFieldsToValues(T instance, Object[] row, NormalizedString[] headers, int[] indexes,
			boolean columnsReordered) {
	 ...
	 *
	 * @param bean           an instance of the type defined in this class
	 *                       constructor.
	 * @param headers        All field names used to produce records in a given
	 *                       destination. May be null if no headers have been
	 *                       defined in
	 *                       {@link CommonSettings#getHeaders()}
	 * @param indexesToWrite The indexes of the headers that are actually being
	 *                       written. May be null if no fields have been selected
	 *                       using
	 *                       {@link CommonSettings#selectFields(String...)} or
	 *                       {@link CommonSettings#selectIndexes(Integer...)}
	 *
	...
	/**
	 * Returns the class of the annotated java bean instances that will be
	 * manipulated by this processor.
	 *
	 * @return the class of the annotated java bean instances that will be
	 *         manipulated by this processor.
	 */
	...
	private void populateTargetMaps(Map<Object, Integer> targetCounts, Map<Object, String> targetSources) {
		for (FieldMapping field : parsedFields) {
			Object outputColumn = field.getIndex() == -1 ? field.getFieldName()
					: NormalizedString.valueOf("Column #" + field.getIndex());

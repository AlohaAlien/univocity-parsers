 ...
 ******************************************************************************/
package com.bupt.se.common.processor;
...
/**
 * A concrete implementation of {@link RowWriterProcessorSwitch} that allows
 * switching among different implementations of
 * {@link RowWriterProcessor} based on values found on rows to be written to an
 * output
 */
public class OutputValueSwitch extends RowWriterProcessorSwitch {
	...
	/**
	 * Creates a switch that will analyze the first column of output rows to
	 * determine which
	 ...
	 *                    which {@link RowWriterProcessor} to use for each output
	 *                    row.
	 */
	 ...
	 * Creates a switch that will analyze a column of output rows to determine which
	 * {@link RowWriterProcessor} to use.
	 * When no column index is defined, the switch will use the first column of any
	 * input rows sent for writing.
	 *
	 * @param headerName the column name whose value will be used to determine which
	 *                   {@link RowWriterProcessor} to use for each output row.
	 */
	 ...
	 * {@link RowWriterProcessor} to use.
	 *
	 * @param headerName  the column name whose value will be used to determine
	 *                    which {@link RowWriterProcessor} to use for each output
	 *                    row.
	 * @param columnIndex the position of an input column to use to perform the
	 *                    switch when no headers are available.
	 */
	 ...
	 *
	 * @param comparator the comparator to use for matching values in the output
	 *                   column with the values provided in
	 *                   {@link #addSwitchForValue(Object, RowWriterProcessor)}
	 */
	...
	/**
	 * Defines a default {@link RowWriterProcessor} implementation to use when no
	 * matching value is found in the output row.
	 *
	 * @param rowProcessor the default row writer processor implementation
	 * @param headersToUse the (optional) sequence of headers to assign to the given
	 *                     row writer processor
	 */
	 ...
	 *                     the given row writer processor
	 */
	...
	/**
	 * Associates a {@link RowWriterProcessor} implementation with an expected value
	 * to be matched in the column provided in the constructor of this class.
	 *
	 * @param value        the value to match against the column of incoming output
	 *                     rows and trigger the usage of the given row writer
	 *                     processor implementation.
	 * @param rowProcessor the row writer processor implementation to use when the
	 *                     given value matches with the contents of the column
	 ...
	 * to be matched in the column provided in the constructor of this class.
	 *
	 * @param beanType     the type of annotated java beans whose instances will be
	 *                     used to trigger the usage of a different format.
	 * @param headersToUse the (optional) sequence of headers to assign to the given
	 *                     row writer processor
	 * @param <T>          the type of annotated java beans whose instances will be
	 *                     used to trigger the usage of a different format.
	 */
	 ...
	 * to be matched in the column provided in the constructor of this class.
	 *
	 * @param beanType the type of annotated java beans whose instances will be used
	 *                 to trigger the usage of a different format.
	 * @param <T>      the type of annotated java beans whose instances will be used
	 *                 to trigger the usage of a different format.
	 */
	...
	/**
	 * Returns the column index whose values will be used to switching from a row
	...
	private static class Switch {
		...
		private Switch(RowWriterProcessor<Object[]> processor, String[] headers, int[] indexes, Object value,
				Class<?> type) {
			if (type != null) {
				...
				if (headers == null && indexes == null) {
					...
					indexes = ArgumentUtils.toIntArray(
							Arrays.asList(AnnotationHelper.getSelectedIndexes(type, MethodFilter.ONLY_GETTERS)));
				}
			}
			this.processor = processor;
			this.headers = headers == null || headers.length == 0 ? null
					: NormalizedString.toIdentifierGroupArray(headers);

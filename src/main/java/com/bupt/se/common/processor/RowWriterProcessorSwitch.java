...
/**
 * A special {@link RowWriterProcessor} implementation that combines and allows
 * switching among different
 * RowWriterProcessors. Concrete implementations of this class
 * are expected to implement the {@code switchRowProcessor(T)} method and
 * analyze the input row
 * to determine whether or not the current {@link RowWriterProcessor}
 * implementation must be changed to handle a special
 * circumstance (determined by the concrete implementation) such as a different
 * row format.
 *
 * When the row writer processor is switched, the
 * {@link #rowProcessorSwitched(RowWriterProcessor, RowWriterProcessor)}
...
public abstract class RowWriterProcessorSwitch implements RowWriterProcessor<Object> {
	...
	/**
	 * Analyzes an output row to determine whether or not the row writer processor
	 * implementation must be changed
	 *
	 * @param row a record with data to be written to the output
	 *
	 * @return the row writer processor implementation to use. If it is not the same
	 *         as the one used by the previous row,
	...
	/**
	 * Returns the headers in use by the current row writer processor
	 * implementation, which can vary among row writer processors.
	 * If {@code null}, the headers defined in
	 * {@link CommonWriterSettings#getHeaders()} will be returned.
	 *
	...
	/**
	 * Returns the indexes in use by the current row writer processor
	 * implementation, which can vary among row writer processors.
	 * If {@code null}, the indexes of fields that have been selected using
	...
	/**
	 * Notifies a change of row writer processor implementation. Users are expected
	 * to override this method to receive the notification.
	 *
	 * @param from the row writer processor previously in use
	 * @param to   the new row writer processor to use to continue processing the
	 *             output rows.
	 */
	...
	public Object[] write(Object input, NormalizedString[] headers, int[] indicesToWrite) {
		...
		if (processor == null) {
			DataProcessingException ex = new DataProcessingException(
					"Cannot find switch for input. Headers: {headers}, indices to write: "
							+ Arrays.toString(indicesToWrite) + ". " + describeSwitch());

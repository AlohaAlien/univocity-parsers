...
/**
 * A {@code DataProcessingException} is an error thrown during the processing of
 * a record successfully parsed.
 * This type of error usually indicates that the input text has been parsed
 * correctly, but the subsequent
 * transformations applied over the input (generally via a {@link RowProcessor}}
 * failed.
 *
...
public class DataProcessingException extends TextParsingException {
	...
	/**
	 * Creates a new exception with an error message and the row that could not be
	 * processed.
	 *
	 * @param message the error message
	...
	/**
	 * Creates a new exception with an error message, the row that could not be
	 * processed, and the error cause.
	 *
	 * @param message the error message
	 * @param row     the row that could not be processed.
	 * @param cause   the cause of the error
	...
	/**
	 * Creates a new exception with an error message and the column that could not
	 * be processed.
	 *
	 * @param message     the error message
	...
	/**
	 * Creates a new exception with an error message, the column that could not be
	 * processed
	...
	/**
	 * Associates a label in the exception message (idenfied in curly braces) with a
	 * value being processed when the error occurred.
	 * Used for formatting the exception message
	 *
	 * @param label a label in the exception message - any string enclosed by curly
	 *              braces.
	 * @param value the value being processed when error occurred, that should be
	 *              replaced by the label in the exception message.
	 */
	...
	/**
	 * Updates the exception with the record being processed when the error
	 * occurred.
	 *
	...
	/**
	 * Returns a flag indicating whether this error is fatal and the process must
	 * stop as it is impossible to proceed.
	 *
	 * @return a flag indicating whether this error is fatal and the process must
	 *         stop as it is impossible to proceed.
	 */
	 ...
	 * {@link ProcessorErrorHandler} again.
	 * 
	 * @param handler the {@link ProcessorErrorHandler} used to handle this
	 *                exception.
	 */
	public final void markAsHandled(ProcessorErrorHandler handler) {
		this.handled = handler != null && !(handler instanceof NoopProcessorErrorHandler)
				&& !(handler instanceof NoopRowProcessorErrorHandler);
	}
	 ...
	 *
	 * @return {@code true} if this exception has been handled to a user-provided
	 *         {@link ProcessorErrorHandler},

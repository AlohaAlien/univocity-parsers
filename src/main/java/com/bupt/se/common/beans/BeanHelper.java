...
package com.bupt.se.common.beans;
...
/**
 * Helper class used to obtain property descriptors from annotated java beans
 * whose values are set via reflection.
 * This class was implemented to eliminate direct compile-time dependency with
 * {@link java.beans.Introspector} and
 * other classes in the {@code java.beans.*} package. This is required to allow
 * Android developers to use univocity-parsers.
 * Android developers should add have <a href=
 * "https://code.google.com/p/openbeans/downloads/detail?name=openbeans-1.0.jar">openbeans-1.0.jar</a>
 * in their classpath to be able to use univocity-parsers.
 *
 * When available, the classes from package {@code com.googlecode.openbeans.*}
 * will be used, otherwise the
 * bean introspection classes classes from {@code java.beans.*} package will be
 * loaded.
 *
 * If everything fails, then the parser will try to manipulate fields in
 * annotated java beans directly, instead
...
public final class BeanHelper {
	 ...
	 * @return an array of all property descriptors of the given class. Might be
	 *         empty.
	 */

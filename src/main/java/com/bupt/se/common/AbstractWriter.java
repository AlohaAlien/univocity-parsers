...
/**
 * The AbstractWriter class provides a common ground for all writers in
 * univocity-parsers.
 *
 * It handles all settings defined by {@link CommonWriterSettings}, and
 * delegates the writing algorithm implementation to its subclasses through the
 * abstract
 * method {@link AbstractWriter#processRow(Object[])}
 *
 * The following (absolutely required) attributes are exposed to subclasses:
 * <ul>
 * <li><b>appender (<i>{@link WriterCharAppender}</i>):</b> the character writer
 * that appends characters from a given input into an internal buffer</li>
 * </ul>
 *
 * @param <S> The specific writer settings configuration class, which can
 *            potentially provide additional configuration options supported by
 *            the writer
 *            implementation.
 *
 * @author Univocity Software Pty Ltd -
 *         <a href="mailto:parsers@univocity.com">parsers@univocity.com</a>
 * @see com.bupt.se.csv.CsvWriter
 * @see com.bupt.se.csv.CsvWriterSettings
 * @see com.bupt.se.fixed.FixedWidthWriter
 * @see com.bupt.se.fixed.FixedWidthWriterSettings
 * @see com.bupt.se.tsv.TsvWriter
 * @see com.bupt.se.tsv.TsvWriterSettings
 * @see com.bupt.se.common.input.WriterCharAppender
 * @see com.bupt.se.common.processor.RowWriterProcessor
 */
public abstract class AbstractWriter<S extends CommonWriterSettings<?>> {
	 ...
	 * {@link CommonWriterSettings}. The AbstractWriter requires its configuration
	 * to be
	 * properly initialized.
	 * <strong>Important: </strong> by not providing an instance of
	 * {@link java.io.Writer} to this constructor, only the operations that write to
	 * Strings are
	 * available.
	 *
	 ...
	 * properly initialized.
	 *
	 * @param file     the output file that will be written with the format-specific
	 *                 records as defined by subclasses of {@link AbstractWriter}.
	 * @param settings the writer configuration
	 */
	 ...
	 * properly initialized.
	 *
	 * @param output   the output stream that will be written with the
	 *                 format-specific records as defined by subclasses of
	 *                 {@link AbstractWriter}.
	 * @param settings the writer configuration
	 */
	 ...
	 * properly initialized.
	 *
	 * @param writer   the output resource that will receive the format-specific
	 *                 records as defined by subclasses of {@link AbstractWriter}.
	 * @param settings the writer configuration
	 */
	public AbstractWriter(Writer writer, S settings) {
		...
		this.appender = new WriterCharAppender(settings.getMaxCharsPerColumn(), "", whitespaceRangeStart,
				settings.getFormat());
		this.rowAppender = new WriterCharAppender(settings.getMaxCharsPerColumn(), "", whitespaceRangeStart,
				settings.getFormat());
	...
	/**
	 * Initializes the concrete implementation of this class with format-specific
	 * settings.
	 *
	...
	private void updateIndexesToWrite(CommonSettings<?> settings) {
		...
		if (selector != null) {
			if (headers != null && headers.length > 0) {
				...
				indexesToWrite = selector.getFieldIndexes(new NormalizedString[rowLength]); // generates a dummy header
																							// array - only the indexes
																							// matter so we are good
				if (columnReorderingEnabled) { // column reordering enabled?
					int size = ArgumentUtils.removeAll(indexesToWrite, -1).length;
					outputRow = new Object[size];
				} else {
	 ...
	 * change during the writing process and their values need be allocated to
	 * specific columns.
	 *
	...
	/**
	 * Updates the selection of fields to exclude when writing. This is useful if
	 * the input rows
	 * change during the writing process and their values need be allocated to
	 * specific columns.
	 *
	 ...
	 * specific columns.
	 *
	 * @param fieldIndexesToExclude the selection of fields to exclude from the
	 *                              output.
	 */
	 ...
	 *
	 * The AbstractWriter handles the initialization and processing of the output
	 * until it is ready to be written (generally, reorganizing it and passing it on
	 * to a {@link RowWriterProcessor}).
	 * It then delegates the record to the writer-specific implementation defined by
	 * {@link #processRow(Object[])}. In general, an implementation of {@link
	 * AbstractWriter#processRow(Object[])} will perform the following steps:
	 * <ul>
	 * <li>Iterate over each object in the given input and convert it to the
	 * expected String representation.</li>
	 * <li>The conversion <b>must</b> happen using the provided
	 * {@link AbstractWriter#appender} object. The an individual value is processed,
	 * the {@link AbstractWriter#appendValueToRow()} method must be called.
	 * This will clear the accumulated value in {@link AbstractWriter#appender} and
	 * add it to the output row.</li>
	 * <li>Format specific separators and other characters must be introduced to the
	 * output row using {@link AbstractWriter#appendToRow(char)}</li>
	 * </ul>
	 * Once the {@link #processRow(Object[])} method returns, a row will be written
	 * to the output with the processed information, and a newline will be
	 * automatically written after the given contents, unless this is a
	 * {@link com.bupt.se.fixed.FixedWidthWriter} whose
	 * {@link FixedWidthWriterSettings#getWriteLineSeparatorAfterRecord()} evaluates
	 * to {@code false}. The newline character sequence will conform to what is
	 * specified in {@link Format#getLineSeparator()}
	 * This cycle repeats until the writing process is stopped by the user or an
	 * error happens.
	 * In case of errors, the unchecked exception {@link TextWritingException} will
	 * be thrown and all resources in use will be closed automatically. The
	 * exception should contain the cause and more information about the output
	 * state when the error happened.
	 *
	 * @param row the data to be written to the output in the expected format.
	 *
	 * @see com.bupt.se.common.input.CharAppender
	 * @see com.bupt.se.common.CommonWriterSettings
	 */
	protected abstract void processRow(Object[] row);

	/**
	 * Appends the processed sequence of characters in
	 * {@link AbstractWriter#appender} to the output row.
	 */
	 ...
	 * A {@link TextWritingException} will be thrown if no headers were defined or
	 * if records were already written to the output.
	 */
	 ...
	 * {@link RowWriterProcessor} provided by
	 * {@link CommonWriterSettings#getRowWriterProcessor()}, and
	 * writes them, then finally and closes the output
	 * A {@link TextWritingException} will be thrown if no
	 * {@link RowWriterProcessor} is provided by
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 *
	 * @param allRecords the records to be transformed by a
	 *                   {@link RowWriterProcessor} and then written to the output
	 */
	 ...
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 *
	 * @param records the records to be transformed by a {@link RowWriterProcessor}
	 ...
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 *
	 * @param records the records to transformed by a {@link RowWriterProcessor} and
	 ...
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 *
	 * @param record the information of a single record to be transformed by a
	 *               {@link RowWriterProcessor} and then written to the output
	 */
	...
	public final void processRecord(Object record) {
		if (this.writerProcessor == null) {
			...
			String message = "Cannot process record '" + recordDescription
					+ "' without a writer processor. Please define a writer processor instance in the settings or use the 'writeRow' methods.";
	 ...
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecordsAndClose(Iterable)} for that.
	 *
	 ...
	 * Iterates over all records, writes them and closes the output.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecordsAndClose(Object[])} for that.
	 *
	 * @param allRows the rows to be written to the output
	 */
	 ...
	 * The output will remain open for further writing.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecords(Object[])} for that.
	 *
	 ...
	 * The output will remain open for further writing.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecords(Iterable)} for that.
	 *
	 * @param <C>  Collection of objects containing values of a row
	 * @param rows the rows to be written to the output
	 */
	 ...
	 * The output will remain open for further writing.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecord(Object)} for that.
	 *
	 ...
	 * Writes the data given for an individual record.
	 * The output will remain open for further writing.
	 * If the given data is null or empty, and
	 * {@link CommonSettings#getSkipEmptyLines()} is true, the input will be just
	 * ignored.
	 * If {@link CommonSettings#getSkipEmptyLines()} is false, then an empty row
	 * will be written to the output (as specified by {@link
	 * AbstractWriter#writeEmptyRow()}).
	 * In case of any errors, a {@link TextWritingException} will be thrown and the
	 * {@link java.io.Writer} given in the constructor will be closed.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecord(Object)} for that.
	 *
	 * @param row the information of a single record to be written to the output
	 ...
	 * A newline will automatically written after the given contents, unless this is
	 * a
	 * {@link com.bupt.se.fixed.FixedWidthWriter} whose
	 * {@link FixedWidthWriterSettings#getWriteLineSeparatorAfterRecord()} evaluates
	 * to {@code false}.
	 * The newline character sequence will conform to what is specified in
	 * {@link Format#getLineSeparator()}
	...
	/**
	 * Used when fields were selected and the input rows have a different order than
	 * the output. This method
	 * fills the internal #outputRow array with the values provided by the user in
	 * the correct order.
	 *
	 * @param row user-provided data which has to be rearranged to the expected
	 *            record sequence before writing to the output.
	 */
	...
	/**
	 * Writes the accumulated value of a record to the output, followed by a
	 * newline, and increases the record count.
	 * The newline character sequence will conform to what is specified in
	 * {@link Format#getLineSeparator()}
	 * The contents of {@link AbstractWriter#rowAppender} depend on the concrete
	 * implementation of {@link AbstractWriter#processRow(Object[])}
	 */
	...
	/**
	 * Identifies the starting character index of a value being written if leading
	 * whitespaces are to be discarded.
	 * <b>Implementation note</b> whitespaces are considered all characters where
	 * {@code ch <= ' '} evaluates to {@code true}
	 *
	 * @param whitespaceRangeStart starting range after which characters will be
	 ...
	 * An IllegalStateException will be thrown in case of any errors, and the writer
	 * will be closed.
	 */
	...
	public final void close() {
		...
		if (this.partialLineIndex != 0) {
			throw new TextWritingException(
					"Not all values associated with the last record have been written to the output. " +
	...
	/**
	 * In case of any exceptions, a {@link TextWritingException} is thrown, and the
	 * output {@link java.io.Writer} is closed.
	 *
	 ...
	 *
	 * @param message          Description of the error
	 * @param recordCharacters characters used to write to the output at the time
	 *                         the exception happened
	 * @param cause            the exception to be wrapped by a
	 *                         {@link TextWritingException}
	 */
	private TextWritingException throwExceptionAndClose(String message, String recordCharacters, Throwable cause) {
		try {
			if (cause instanceof NullPointerException && writer == null) {
				message = message + " No writer provided in the constructor of " + getClass().getName()
						+ ". You can only use operations that write to Strings.";
			}
	 ...
	 *
	 * @param message      Description of the error
	 * @param recordValues values used to write to the output at the time the
	 *                     exception happened
	 * @param cause        the exception to be wrapped by a
	 *                     {@link TextWritingException}
	 */
	 ...
	 * {@code String}
	 * <ul>
	 * <li>If the object is null, then {@link AbstractWriter#nullValue} is
	 * returned.</li>
	 * <li>If the String representation of this object is an empty String, then
	 * {@link AbstractWriter#emptyValue} is returned</li>
	 * </ul>
	 *
	...
	/**
	 * Writes a sequence of values to a row in memory. Subsequent calls to this
	 * method will add the given values in a new column of the same row, until
	 * {@link
	...
	/**
	 * Writes a sequence of Strings to a row in memory. Subsequent calls to this
	 * method will add the given values in a new column of the same row, until
	 * {@link
	 * #writeValuesToRow} is called to flush
	 * all values accumulated and effectively write a new record to the output
	...
	/**
	 * Writes a value to a row in memory. Subsequent calls to this method will add
	 * the given values in a new column of the same row, until {@link
	 * #writeValuesToRow} is called to flush
	 * all values accumulated and effectively write a new record to the output
	 *
	 ...
	 * {@link #addValues(Object...) or #writeValue()} to a new record in the output.
	 */
	...
	public final void addValue(int index, Object value) {
		if (index >= partialLine.length) {
			throw throwExceptionAndClose("Cannot write '" + value + "' to index '" + index
					+ "'. Maximum number of columns (" + partialLine.length + ") exceeded.", new Object[] { value },
					null);
		}
	 ...
	 * all values accumulated and effectively write a new record to the output
	 *
	 * @param headerName the name of the column of the new row that should receive
	 *                   the value.
	 * @param value      the value to be written
	 */
	...
	private final void addValue(NormalizedString[] headersInContext, NormalizedString headerName,
			boolean ignoreOnMismatch, Object value) {
	 ...
	 * {@link #headers} array defined in this writer
	 *
	 * @param headersInContext headers currently in use (they might change).
	 * @param headerName       the name of the header whose position will be
	 *                         identified
	 * @param ignoreOnMismatch flag indicating that if the header is not found, no
	 *                         exception is to be thrown, and -1 should be returned
	 *                         instead.
	 *
	 * @return the position of the given header, or -1 if it's not found when
	 *         ignoreOnMismatch is set to {@code true}
	 */
	private int getFieldIndex(NormalizedString[] headersInContext, NormalizedString headerName,
		...
		if (index == null) {
			if (headersInContext == null) {
				throw throwExceptionAndClose(
						"Cannot calculate position of header '" + headerName + "' as no headers were defined.", null);
			}
			index = ArgumentUtils.indexOf(NormalizedString.toArray(headersInContext),
					NormalizedString.valueOf(headerName));
			if (index == -1) {
				if (!ignoreOnMismatch) {
					throw throwExceptionAndClose("Header '" + headerName + "' could not be found. Defined headers are: "
							+ Arrays.toString(headersInContext) + '.', null);
				}
			}
	 ...
	 * {@link #addValues(Object...) or #writeValue()}.
	 */
	...
	/**
	 * Writes the headers defined in {@link CommonSettings#getHeaders()} to a
	 * {@code String}
	 *
	 * @return a formatted {@code String} containing the headers defined in
	 ...
	 * Writes the given collection of headers to a {@code String}
	 * A {@link TextWritingException} will be thrown if no headers were defined or
	 * if records were already written to a {@code String}
	 *
	 * @param headers the headers to write to a {@code String}
	 *
	 * @return a formatted {@code String} containing the given headers
	 ...
	 *
	 * @param records the records to be transformed by a {@link RowWriterProcessor}
	 *                and then written to a {@code List} of {@code String}.
	 *
	 * @return a {@code List} containing the information transformed from the given
	 *         records as formatted {@code String}s
	 */
	 ...
	 *
	 * @param record the information of a single record to be transformed by a
	 *               {@link RowWriterProcessor} and then written to a
	 *               {@code String}.
	 *
	 * @return a formatted {@code String} containing the information transformed
	...
	public final String processRecordToString(Object record) {
		if (this.writerProcessor == null) {
			throw throwExceptionAndClose("Cannot process record '" + record
					+ "' without a writer processor. Please define a writer processor instance in the settings or use the 'writeRow' methods.");
		}
	...
	/**
	 * Iterates over all records and writes them to a {@code List} of
	 * {@code String}.
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}. Use
	 * {@link AbstractWriter#processRecords(Object[])} for that.
	 *
	 ...
	 * {@link AbstractWriter#processRecord(Object)} for that.
	 *
	 * @param row the information of a single record to be written to a
	 *            {@code String}
	 *
	 * @return a formatted {@code String} containing the information of the given
	 *         record
	 */
	 ...
	 * Writes the data given for an individual record to a {@code String}.
	 * If the given data is null or empty, and
	 * {@link CommonSettings#getSkipEmptyLines()} is true, {@code null} will be
	 * returned
	...
	private Object[] adjustRowLength(Object[] row) {
		if (outputRow != null) {
			...
			if (usingSwitch) {
				row = expand(row, dummyHeaderRow == null ? -1 : dummyHeaderRow.length,
						headers == null ? null : headers.length);
				dummyHeaderRow = null;
			} else {
	...
	/**
	 * Writes the contents accumulated in an internal in-memory row (using
	 * {@link #addValues(Object...) or #addValue()} as a {@code String}
	 *
	 * @return a formatted {@code String} containing the information accumulated in
	 *         the internal in-memory row.
	 */
	...
	/**
	 * Writes the contents accumulated in an internal in-memory row (using
	 * {@link #addValues(Object...) or #addValue()} to a new record in the output.
	 * The objects added to this row will be processed with the
	 * {@link RowWriterProcessor} provided by
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 * The output will remain open for further writing.
	 * A {@link TextWritingException} will be thrown if no
	...
	/**
	 * Writes the contents accumulated in an internal in-memory row (using
	 * {@link #addValues(Object...) or #addValue()} to a {@code String}
	 * The objects added to this row will be processed with the
	 * {@link RowWriterProcessor} provided by
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 * The output will remain open for further writing.
	 ...
	 * {@link CommonWriterSettings#getRowWriterProcessor()}.
	 *
	 * @return a formatted {@code String} containing the result produced by the
	 *         {@link RowWriterProcessor} using the values accumulated in internal
	 *         in-memory row.
	 */
	...
	/**
	 * Writes values from an implementation of {@link java.util.Map} to a partial
	 * output record, ready to be
	 * written to the output.
	 *
	 * Values will be stored under a column identified by the headers. If no headers
	 * are defined,
	 * the keys of the map will be used to initialize an internal header row.
	 *
	 * A map of headers can be optionally provided to assign a name to the keys of
	 * the input map. This is useful when
	 * the input map has keys will generate unwanted header names.
	 *
	 * @param headerMapping an optional map associating keys of the rowData map with
	 *                      expected header names
	 * @param rowData       the data to be written. Its keys will be used to form a
	 *                      header row in case no headers are available.
	 ...
	 *
	 * @param map  the input map whose keys will be used to generate headers for the
	 *             output.
	 * @param keys indicates whether to take the map keys or values to build the
	 *             header rows.
	 */
	...
	/**
	 * Writes the values of a given map to a {@code String} formatted to according
	 * to the specified output format.
	 *
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}.
	 *
	 * @param rowData the map whose values will be used to generate a
	 *                {@code String}.
	 *
	 * @return a {@code String} containing the given data as a formatted
	 *         {@code String}
	 */
	 ...
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}.
	 *
	 * @param headerMapping a mapping associating the keys of the input map to their
	 *                      corresponding header names.
	 * @param rowData       the map whose values will be used to generate a
	 *                      {@code String}.
	 ...
	 * @param headerMapping a mapping associating the keys of the input map to their
	 *                      corresponding header names.
	 * @param rowData       the map whose values will be used to generate a new
	 *                      record
	 * @param <K>           the key type
	 */
	...
	/**
	 * Writes the values of a given map to a {@code List} of {@code String}
	 * formatted to according to the specified output format.
	 *
	 * Each value is expected to be iterable and the result of this method will
	 * produce the number of records equal to the longest iterable.
	 *
	 * A new record will be created each time at least one
	 * {@link Iterator#hasNext()} returns {@code true}. {@code Null} will be written
	 * when a iterator has been fully read.
	 *
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}.
	 *
	 * @param rowData the map whose values will be used to generate a {@code List}
	 *                of {@code String}.
	 * @param <K>     the key type
	 * @param <I>     the iterable type
	 *
	 * @return a {@code List} of formatted {@code String}, each {@code String}
	 ...
	 * <b>Note</b> this method will not use the {@link RowWriterProcessor}.
	 *
	 * @param rowData the map whose values will be used to generate a number of
	 *                output records
	 * @param <K>     the key type
	 * @param <I>     the iterable type
	 */
	 ...
	 *         element of the iterators in the map.
	 */
	public final <K, I extends Iterable<?>> List<String> writeRowsToString(Map<K, String> headerMapping,
			Map<K, I> rowData) {
	 ...
	 * @param headerMapping a mapping associating the keys of the input map to their
	 *                      corresponding header names.
	 * @param rowData       the map whose values will be used to generate a number
	 *                      of output records
	 * @param <K>           the key type
	 * @param <I>           the iterable type
	 */
	 ...
	 * @param rowData       the map whose values will be used to generate a
	 *                      {@code List} of {@code String}.
	 * @param outputList    an output {@code List} to fill with formatted
	 *                      {@code String}s, each {@code String} representing
	...
	/**
	 * Writes the values of a given map to multiple output records and closes the
	...
	/**
	 * Processes the values of a map using the {@link RowWriterProcessor} provided
	 * by {@link CommonWriterSettings#getRowWriterProcessor()}
	 * and writes the result into {@code String} formatted according to the
	 ...
	 * {@link RowWriterProcessor} provided by
	 * {@link CommonWriterSettings#getRowWriterProcessor()}
	 * and writes the result into a {@code List} of {@code String} formatted to
	 ...
	 *         element of the iterators in the map.
	 */
	public final <K, I extends Iterable<?>> List<String> processRecordsToString(Map<K, String> headerMapping,
			Map<K, I> rowData) {
		List<String> writtenRows = new ArrayList<String>();
	 ...
	 * @param <I>           the iterable type
	 */
	public final <K, I extends Iterable<?>> void processRecordsAndClose(Map<K, String> headerMapping,
			Map<K, I> rowData) {
		try {
	...
	/**
	 * Checks whether the writer can remove trailing/leading whitespaces from a
	 * value being written.
	 * Applies to headers where names can be conflicting if trimmed for example
	 * {@code ' a '} and
	 * {@code 'a'} will become the same value if the spaces are removed)
	 *
	 * @param fieldIndex index of the field to be written.
	 *
	 * @return {@code true} if the value being written is not a header name, or it
	 *         is a header name that won't
	 *         conflict with another header if its surrounding whitespaces are
	 *         trimmed.
	 */

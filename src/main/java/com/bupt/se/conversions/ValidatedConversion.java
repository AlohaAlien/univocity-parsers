...
public class ValidatedConversion implements Conversion<Object, Object> {
	...
	public ValidatedConversion(boolean nullable, boolean allowBlanks, String[] oneOf, String[] noneOf,
			String regexToMatch, Class[] validators) {
		this.regexToMatch = regexToMatch;
		this.matcher = regexToMatch == null || regexToMatch.isEmpty() ? null
				: Pattern.compile(regexToMatch).matcher("");
		...
		this.validators = validators == null || validators.length == 0 ? new Validator[0]
				: instantiateValidators(validators);
	}
	...
	private Validator[] instantiateValidators(Class[] validators) {
		...
		for (int i = 0; i < validators.length; i++) {
			out[i] = (Validator) AnnotationHelper.newInstance(Validator.class, validators[i],
					ArgumentUtils.EMPTY_STRING_ARRAY);
		}
			...
			if (matcher != null && e == null) {
				...
				if (!match) {
					e = new DataValidationException(
							"Value '{value}' does not match expected pattern: '" + regexToMatch + "'");
				}
			}
		}

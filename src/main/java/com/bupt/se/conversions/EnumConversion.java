 ...
 *
 * <p>
 * This class supports multiple types of identification of enumeration
 * constants. For example, you can match the literal ({@link Enum#name()} the
 * ordinal ({@link Enum#ordinal()} or
 * the result of a method defined in your enumeration.
 *
 * <p>
 * The reverse conversion from an enumeration to String (in
 * {@link EnumConversion#revert(Enum)} will return a String using the first
 * {@link EnumSelector} provided in the constructor of this class.
 *
 * @param <T> the enumeration type whose constants will be converted from/to
 *            {@code String}
 * 
...
public class EnumConversion<T extends Enum<T>> extends ObjectConversion<T> {
	...
	/**
	 * Defines a conversion for an enumeration type that will attempt to match
	 * Strings against
	 * the results of the output produced by ({@link Enum#name()},
	 * ({@link Enum#ordinal()} and ({@link Enum#toString()}
	 * of each constant of the given enumeration (@link
	...
	/**
	 * Defines a conversion for an enumeration type that will attempt to match
	 * Strings the list of {@link EnumSelector}s, in the specified order.
	 * Each {@link EnumSelector} identifies which element of each constant of the
	 * enumeration class (@link {@link Class#getEnumConstants()}
	...
	public EnumConversion(Class<T> enumType, T valueIfStringIsNull, String valueIfEnumIsNull, String customEnumElement,
		...
		if (customEnumElement != null) {
			...
			try {
				...
				fieldError = new IllegalStateException("Unable to access custom field '" + customEnumElement
						+ "' in enumeration type " + enumType.getName(), e);
			}
				...
				try {
					try {
						...
						if (!Modifier.isStatic(method.getModifiers())) {
							throw new IllegalArgumentException("Custom method '" + customEnumElement
									+ "' in enumeration type " + enumType.getName() + " must be static");
						}
						if (method.getReturnType() != enumType) {
							throw new IllegalArgumentException(
									"Custom method '" + customEnumElement + "' in enumeration type "
											+ enumType.getName() + " must return " + enumType.getName());
						}
					}
					...
					methodError = new IllegalStateException("Unable to access custom method '" + customEnumElement
							+ "' in enumeration type " + enumType.getName(), e);
				}
				if (method != null) {
					if (method.getReturnType() == void.class) {
						throw new IllegalArgumentException("Custom method '" + customEnumElement
								+ "' in enumeration type " + enumType.getName() + " must return a value");
					}
			...
			if (field == null && method == null) {
				throw new IllegalStateException("No method/field named '" + customEnumElement
						+ "' found in enumeration type " + enumType.getName());
			}
		}

		if (selectorSet.contains(EnumSelector.CUSTOM_FIELD) && selectorSet.contains(EnumSelector.CUSTOM_METHOD)) {
			throw new IllegalArgumentException(
					"Cannot create custom enum conversion using both method and field values");
		}
			...
			for (T constant : constants) {
				...
				if (key != null) {
					if (map.containsKey(key)) {
						throw new IllegalArgumentException("Enumeration element type " + conversionType
								+ " does not uniquely identify elements of " + enumType.getName()
								+ ". Got duplicate value '" + key + "' from constants '" + constant
								+ "' and '" + map.get(key) + "'.");
					}
		...
		switch (conversionType) {
			...
			case CUSTOM_FIELD:
				try {
					...
					throw new IllegalStateException("Error reading custom field '" + customEnumField.getName()
							+ "' from enumeration constant '" + constant + "' of type " + enumType.getName(), e);
				}
			case CUSTOM_METHOD:
				try {
					...
					throw new IllegalStateException(
							"Error reading custom method '" + customEnumMethod.getName()
									+ "' from enumeration constant '" + constant + "' of type " + enumType.getName(),
							e);
				}
			default:
	...
	protected T fromString(String input) {
		...
		if (customEnumMethod != null && customEnumMethod.getParameterTypes().length == 1) {
			try {
				...
				exception = new DataProcessingException("Cannot convert '{value}' to enumeration of type "
						+ enumType.getName() + " using method " + customEnumMethod.getName(), e);
			}
		}
		if (exception == null) {
			exception = new DataProcessingException(
					"Cannot convert '{value}' to enumeration of type " + enumType.getName());
		}

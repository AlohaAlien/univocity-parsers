...
public class CsvParserExamples extends Example {
	...
	public void example004ReadCsvAndConvertValues() throws Exception {
		...
		// converts the values in columns "Make, Model and Description" to lower case,
		// and sets the value "chevy" to null.
		rowProcessor.convertFields(Conversions.toLowerCase(), Conversions.toNull("chevy")).set("Make", "Model",
				"Description");

		// converts the values at index 0 (year) to BigInteger. Nulls are converted to
		// BigInteger.ZERO.
	...
	public void example006MasterDetail() throws Exception {
		...
		// converts values at in the "Amount" column (position 1 in the file) to
		// integer.
		detailProcessor.convertIndexes(Conversions.toInteger()).set(1);

		// 2nd, Create MasterDetailProcessor to identify whether or not a row is the
		// master row.
		// the row placement argument indicates whether the master detail row occurs
		// before or after a sequence of "detail" rows.
		MasterDetailListProcessor masterRowProcessor = new MasterDetailListProcessor(RowPlacement.BOTTOM,
				detailProcessor) {
			@Override
	...
	public void example007Columns() throws Exception {
		...
		Map<String, List<String>> columnValues = new TreeMap<String, List<String>>(
	...
	public void example010Escaping() throws Exception {
		...
		// but if two backslashes are found before a quote symbol they represent a
		// single slash.
		...
		List<String[]> allRows = parser
				.parseAll(new InputStreamReader(getClass().getResourceAsStream("/examples/escape.csv"), "UTF-8"));
	...
	public void example011ErrorHandling() throws Exception {
		...
		// ##CODE_START
		BeanListProcessor<AnotherTestBean> beanProcessor = new BeanListProcessor<AnotherTestBean>(
				AnotherTestBean.class);
		settings.setProcessor(beanProcessor);

		// Let's set a RowProcessorErrorHandler to log the error. The parser will keep
		// running.
		settings.setProcessorErrorHandler(new RowProcessorErrorHandler() {
			...
			public void handleError(DataProcessingException error, Object[] inputRow, ParsingContext context) {
				...
				println(out, "Error details: column '" + error.getColumnName() + "' (index " + error.getColumnIndex()
						+ ") has value '" + inputRow[error.getColumnIndex()] + "'");
			}
		});
	...
	public void example011ErrorHandlingWithRetry() {
		...
		settings.setProcessorErrorHandler(new RetryableErrorHandler<ParsingContext>() {
			...
			public void handleError(DataProcessingException error, Object[] inputRow, ParsingContext context) {
				println(out, "Error processing row: " + Arrays.toString(inputRow));
				println(out, "Error details: column '" + error.getColumnName() + "' (index " + error.getColumnIndex()
						+ ") has value '" + inputRow[error.getColumnIndex()] + "'. Setting it to null");
	...
	public void example012FormatAutodetection() throws Exception {
		...
		// ##CODE_START
		// turns on automatic detection of line separators, column separators, quotes &
		// quote escapes
		...
		// Then, the same data but in European style (column separator is ; and decimals
		// are separated by ,). We also escaped quotes with \ instead of using double
		// quotes
	...
	public void example013ParseKeepingEscapeSequences() throws Exception {
		...
		// let's parse the file seen in the previous example, where quotes are escaped
		// using \
		settings.detectFormatAutomatically();

		// ##CODE_START

		// now we want to keep the escape sequences. We should see the slash before the
		// quotes.
	...
	public void example014ParseMultipleBeansInSingleRow() {
		...
		// The MultiBeanProcessor allows multiple bean instances to be created from a
		// single input record.
		// in this example, we will create instances of TestBean and AnotherTestBean.
		// Here we use a MultiBeanListProcessor which is a convenience class that
		// implements the
		// abstract beanProcessed() method of MultiBeanProcessor to add each instance to
		// a list.;
		MultiBeanListProcessor processor = new MultiBeanListProcessor(TestBean.class, AnotherTestBean.class);

		// one of the records in the input won't be compatible with AnotherTestBean: the
		// field "pending"
		// only accepts 'y' or 'n' as valid representations of true or false. We want to
		// continue processing, let's ignore the error.
		settings.setProcessorErrorHandler(new RowProcessorErrorHandler() {
			@Override
			public void handleError(DataProcessingException error, Object[] inputRow, ParsingContext context) {
				// ignore the error.
		...
		// we can get all beans parsed from the input as a map, where the keys are the
		// bean type associated
		// with a list of corresponding bean instances
		Map<Class<?>, List<?>> beans = processor.getBeans();

		// or we can get the lists of beans processed individually by providing the
		// type:
	...
	public void example015QuoteAndEscapeHandling() {
		...
		// Line separators are normalized by default. This means they are all converted
		...
		// You can disable this behavior to keep the original line separators in parsed
		// values.
		settings.setNormalizeLineEndingsWithinQuotes(false);
		parse("value 1,'line 1\r\nline 2',value 3", settings, "Without normalized line endings");
		// result: [value 1, line 1\r\nline 2, value 3]

		// Values that contain a quote character, but are not enclosed within quotes,
		// are read as-is
		...
		// But if your input comes with escaped quotes, and is not enclosed within
		// quotes you'll get the escape sequence
		...
		parse("value 1,I''m NOT a quoted value,value 3", settings,
				"Value with quote, escaped, not enclosed, processing escape");
		// result: [value 1, I'm NOT a quoted value, value 3]

		// As usual, when you parse values that have escaped characters, such as the
		// quote, you get the unescaped result.
		parse("value 1,'I''m a quoted value',value 3", settings, "Enclosed value, quote escaped");
		// result: [value 1, I'm a quoted value, value 3]

		// But in some cases you might want to get the original text, character by
		// character, including the original escape sequence
		settings.setKeepEscapeSequences(true);
		parse("value 1,'I''m a quoted value',value 3", settings,
				"Enclosed value, quote escaped, keeping escape sequences");
		// result: [value 1, I''m a quoted value, value 3]

		// By default, the parser handles broken quote escapes, so it won't complain
		// about "I'm" not being escaped properly (should be "I''m").
		...
		try {
			...
			println("Quote escape error. Parser stopped after reading: [" + exception.getParsedContent()
					+ "] of column " + exception.getColumnIndex());
		}
		// ##CODE_END
	...
	public void example016ParseReadingComments() {
		...
		String[] row;
		while ((row = parser.parseNext()) != null) {
			// using the getContext method we have access to the parsing context, from where
			// the comments found so far can be accessed
			// let's get the last parsed comment and print it out in front of each parsed
			// row.
		...
		println("\nAll comments found:\n-------------------");
		// The comments() method returns a map of line numbers associated with the
		// comments found in them.
	...
	public void example017ParseMultiSchema() {
		...
		// ##CODE_START
		// Here's a master-detail schema, with rows using different formats. Rows
		// starting with MASTER are master rows
		...
		// We are going to collect all MASTER records using a processor for lists of
		// Object[]
		...
		// We create a switch based on the first column of the input. Values of each
		// column of each row
		// will be compared against registered processors that "know" how to parse the
		// input row.
		InputValueSwitch inputSwitch = new InputValueSwitch(0) {

			// the input value switch allows you to override the "rowProcessorSwitched" so
			// you can handle
			// what should happen when a different type of row is found in the input.
			public void rowProcessorSwitched(RowProcessor from, RowProcessor to) {
				// let's add associate the list of parsed Car instances to the MASTER record
				// parsed before.
				// when the current row processor is not the "carProcessor" anymore, it means we
				// parsed all cars under a MASTER record
				if (from == carProcessor) {
					...
					// clear the list of cars so it's empty for the next batch of cars under another
					// master row.
		...
		// Rows whose value at column 0 is "MASTER" will be processed using the
		// masterElementProcessor.
		inputSwitch.addSwitchForValue("MASTER", masterElementProcessor, 1, 2); // we are not interested in the "MASTER"
																				// value at column 0, let's select which
																				// columns to get values from

		// Car records don't have an identifier in their rows. In this case the input
		// switch will use a "default" processor
		// to handle the input. All records that don't start with "SUPER" are Car
		// records, we use the car processor as the default.
		inputSwitch.setDefaultSwitch(carProcessor, "year", "make", "model", "price"); // the header list here matches
																						// the fields annotated in the
																						// Car class.

		// All we need now is to set row processor of our parser, which is the input
		// switch.
		CsvParserSettings settings = new CsvParserSettings();
	...
	public void example018ParseAllRecords() throws Exception {
		...
		// ##CODE_START
		// configure to grab headers from file. We want to use these names to get values
		// from each record.
		settings.setHeaderExtractionEnabled(true);
		// creates a CSV parser
		CsvParser parser = new CsvParser(settings);
	...
	public void example019IterateOverRecords() throws Exception {
		...
		// among many other things, we can set default values of one ore more columns in
		// the record metadata.

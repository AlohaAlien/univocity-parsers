...
public class FixedWidthParserExamples extends Example {
	...
	public void example003ReadFixedWidthWithRowProcessor() throws Exception {
		...
		// flag to consider the first parsed row as the headers of each column in the
		// file.
	...
	public void example007BatchedColumns() throws Exception {
		...
		// Here we set the batch size to 3, meaning we'll get the column values of at
		// most 3 rows in each batch.
	...
	public void example009ParseWithLookahead() throws Exception {
		...
		// We are going to parse the multi_schema.txt file, with a lookahead value in
		...
		accountFields.addField("ID", 7, FieldAlignment.RIGHT, '0'); // here the account ID will be prefixed by the
																	// lookahead value
		accountFields.addField("Bank", 4);
		accountFields.addField("AccountNumber", 10);
		accountFields.addField("Swift", 7);

		// If a record starts with C#, it's a client record, so we associate "C#" with
		// the client format
	...
	public void example010ParseWithDefaultAndLookahead() throws Exception {
		// Here's the format used for client accounts:
		FixedWidthFields accountFields = new FixedWidthFields();
		accountFields.addField("ID", 5, FieldAlignment.RIGHT, '0'); // now, the account fields won't have a lookahead
																	// value.
		accountFields.addField("Bank", 4);
		accountFields.addField("AccountNumber", 10);
		accountFields.addField("Swift", 7);
		...
		// ##CODE_START
		// In some cases the input records might not have a lookahead value. On the
		// multi_schema2.txt file,
		// only client records have a lookahead. If no other lookahead is matched, the
		// parser will switch back to
		// the default field format. Here, the format used by account records will be
		// used as default.
	...
	public void example011ParseWithLookbehind() throws Exception {
		...
		// ##CODE_START
		// We can also specify a lookbehind value to determine which format to use when
		// parsing the input.

		// If a record starts with C#, it's a client record, so we associate "C#" with
		// the client format.
		settings.addFormatForLookahead("C#", clientFields);
		// If a record parsed previously has a C#, but the current doesn't, then we are
		// processing accounts. Let's use the account format.

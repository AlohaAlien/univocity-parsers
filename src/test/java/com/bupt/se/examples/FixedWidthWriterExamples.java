...
public class FixedWidthWriterExamples extends Example {
	...
	public void example001WriteWithAlignmentAndPadding() throws Exception {
		...
		// "id" has length of 5 characters, is aligned to the right and unwritten spaces
		...
		// name and quantity use the default padding defined in the settings (further
		// below).
		fields.addField("name", 15, FieldAlignment.LEFT);
		fields.addField("quantity", 5, FieldAlignment.CENTER); // "quantity" has more than 5 characters. This header
																// will be truncated.
		...
		// The following settings will override the individual column padding and
		// alignment when writing headers only.
	...
	public void example002WriteWithLookahead() throws Exception {
		...
		// Rows starting with any character then 'A' should be written using the account
		// format
	...
	public void example004WriteWithLookbehind() throws Exception {
		...
		// the client format.
		settings.addFormatForLookahead("C#", clientFields);
		// If a record written previously had a C#, but the current doesn't, then we are
		// writing accounts. Let's use the account format.

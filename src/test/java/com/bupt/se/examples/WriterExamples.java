...
public class WriterExamples extends Example {
	...
	public void example001WriteSimpleCsv() {
		...
		// Writing to an in-memory byte array. This will be printed out to the standard
		// output so you can easily see the result.
		...
		// Here we just tell the writer to write everything and close the given output
		// Writer instance.
	...
	public void example002WriteCsvOneByOne() {
		...
		// we must close the writer. This also closes the java.io.Writer you used to
		// create the CsvWriter instance
		// note no checked exceptions are thrown here. If anything bad happens you'll
		// get an IllegalStateException wrapping the original error.
		writer.close();
		// ##CODE_END
		// Let's just print the resulting CSV
		printAndValidate(csvResult.toString());
	...
	public void example003WriteCsvWithFieldSelection() {
		...
		// if the value is not null, but is empty (e.g. ""), the writer will can be
		// configured to
		...
		// Sets the file headers (used for selection, these values won't be written
		// automatically)
		settings.setHeaders("Year", "Make", "Model", "Description", "Price");

		// Selects which fields from the input should be written. In this case, fields
		// "make" and "model" will be empty
		...
		// writes each row providing values for the selected fields (note the values and
		// field selection order must match)
		writer.writeRow("ac, abs, moon", 3000.00, 1997);
		writer.writeRow("", 4900.00, 1999); // NOTE: empty string will be replaced by "!" as per configured
											// emptyQuotedValue.
	...
	public void example004WriteFixedWidthUsingConversions() {
		...
		// Creates an ObjectRowWriterProcessor that handles annotated fields in the
		// TestBean class.
		...
		// Sets the file headers so the writer knows the correct order when writing
		// values taken from a TestBean instance
		...
		// writes a Fixed Width row with the values set in "bean". Notice that there's
		// no annotated
		// attribute for the "date" column, so it will just be null (an then converted
		// to ? a )
	...
	public void example005WriteFixedWidthUsingAnnotatedBean() {
		...
		// Creates a BeanWriterProcessor that handles annotated fields in the TestBean
		// class.
		...
		// writes a fixed width row with empty values (as nothing was set in the
		// TestBean instance).
		...
		// no annotated
		// attribute for the "date" column, so it will just be null (an then converted
		// to ?, as we have settings.setNullValue("?");)
	...
	public void example006WriteSimpleTsv() {
		...
		// As with the CsvWriter, all you need is to create an instance of TsvWriter
		// with the default TsvWriterSettings.
	...
	public void example007WriteValues() {
		...
		// writes a value to the fourth column (index 3 represents the 4th column - the
		// one with header "D")
	...
	public void example008WriteWithHeaderAnnotation() {
		...
		// We didn't provide a java.io.Writer here, so all we can do is write to Strings
		// (streaming)
		...
		// Calling processRecordToString will write the contents of the bean in a TSV
		// formatted String
	...
	public void example009WriteMapWithTypeConversion() {
		...
		// Using the object row writer processor, we can apply conversions to be applied
		// by default over specific types/
		// ##CODE_START
		ObjectRowWriterProcessor processor = new ObjectRowWriterProcessor();
		...
		// If a null is written to our boolean column, we want to print "N/A", otherwise
		// Y and N for true and false.
		// Note that column-specific conversions also prevent the type-conversions to be
		// applied.
		...
		// Let's write everything into a list of Strings. Each element of the list will
		// be a new row
	...
	public void example010MultiSchemaWrite() {
		// ##CODE_START
		// creates a switch that will use a different row processor for writing a row,
		// based on values at column 0.
		...
		// Field names "type", "field1" and "field2" will be associated with this row
		// processor
		...
		// rows with less values than expected will be expanded, i.e. empty columns will
		// be written
		...
		writer.processRecord(new Object[] { "SUPER", "Value 3" }); // no value for column "field2", an empty column will
																	// be written
	...
	public void example011MultiSchemaWriteWithBeans() {
		...
		writerSwitch.addSwitchForType(Car.class); // you can also define specific fields to write by giving a list of
													// header names/column indexes.
	...
	public void example012WriteMapWithHeaderMapping() {
		...
		// we want to write the data stored in the map above, but the keys don't make
		// any sense as column headers.

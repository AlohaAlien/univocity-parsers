...
public class SettingsExamples extends Example {
	...
	public void example002ColumnSelectionWithNoReordering() {
		...
		// Fields that were not selected will be null, as they are not processed by the
		// parser
	...
	public void example004LotsOfDifferentSettings() {
		...
		// NOTE: when fields are selected, all rows produced will have the exact same
		// number of columns
		...
		// The default is 4096 characters. You need this to avoid OutOfMemoryErrors in
		// case a file
		// does not have a valid format. In such cases the parser might just keep
		// reading from the input
		// until its end or the memory is exhausted. This sets a limit which avoids
		// unwanted JVM crashes.
		parserSettings.setMaxCharsPerColumn(100);

		// for the same reasons as above, this sets a hard limit on how many columns an
		// input row can have.
		// The default is 512.
		...
		// Disables the separate thread that loads the input buffer. By default, the
		// input is going to be loaded incrementally
		// on a separate thread if the available processor number is greater than 1.
		// Leave this enabled to get better performance
	...
	public void example005FixedWidthSettings() {
		// ##CODE_START
		// For the sake of the example, we will not read the last 8 characters (for the
		// Year column).
		// We will also NOT set the padding character to '_' so the output makes more
		// sense for reading
		// and you can see what characters are being processed
		FixedWidthParserSettings parserSettings = new FixedWidthParserSettings(
				new FixedWidthFields(4, 5, 40, 40 /* , 8 */));
		...
		// If a row has more characters than what is defined, skip them until the end of
		// the line.
		parserSettings.setSkipTrailingCharsUntilNewline(true);

		// If a record has less characters than what is expected and a new line is
		// found,
		// this record is considered parsed. Data in the next row will be parsed as a
		// new record.
